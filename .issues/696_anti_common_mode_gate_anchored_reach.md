...
**Status:** T1+T2 DONE (2026-08-28, feat `89f78b09`, bench [690](../.benchmarks/690_anti_common_mode_anchored_reach_goat.md): G1 9+8 unit gates PASS, G2 blend 234 ns / score 6023 ns @ N=1000 — the sub-µs ask honestly missed on the score axis (two exact selection passes, sub-µs at N ≲ 160), G3 default 1978/0/7 + feature-on = default + module tests, G4 0 allocs; both features opt-in). **T3 PASS (2026-08-29)** — riir-ai Bench [794](../riir-ai/.benchmarks/794_clr_anti_common_mode_poc.md), commit riir-ai `50591686c`: 3-arm × 8-seed PoC — panic-only border occupancy 27.1% (the Plan-019 failure reproduced) → panic+gate 0.0% (baseline level, 8/8 seeds) while Bench-010 detection is retained bit-identically (100% non-observer detection, fear field ≡ panic-only arm); per-NPC exposure gate on the collective flee-override channel (median-subtract + band-window), 784–1028 ns/tick, 0 allocs; kick-scalar composition measured-dead (control-pinned). `clr_collective_threat` stays opt-in — re-promotion is an owner gameplay call, now with gate evidence. T4/T5 OPEN.
...
- Bench 794 (riir-ai) — T3 consumer PoC PASS record (`riir-ai/.benchmarks/794_clr_anti_common_mode_poc.md`): 3-arm table + gate composition + substrate-side promotion-hook sketch
