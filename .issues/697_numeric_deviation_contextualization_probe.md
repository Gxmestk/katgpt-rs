...
**Status:** Phase 1 DONE (T1.1–T1.5 + T3.1) — 2026-08-28, commit `5d43d785` (bench: [`.benchmarks/691_numeric_stability_probe_goat.md`](../.benchmarks/691_numeric_stability_probe_goat.md)); **Phase 2 DONE (T2.1+T2.2+T3.2) — 2026-08-29** (`lab.rs`: two-tier golden identity bit-exact single-tile / 1.523e-15 multi-tile bound, all four ordering laws pinned as tests — mantissa ladder, Spearman ρ 0.8892 floor 0.7, tile-area ×2 formats, dim-order swap + bit-identical square control — and the T3.2 schedule: blake3-pinned fit config, 4-row TOL_TABLE_PINNED ×2 headroom, two-length probe through the Phase-1 acceptance rule with the stale-band reject asserted non-vacuous; feature lib 2005/0/9, default 1978/0/7 unchanged, clippy 0 both states). T3.3 open (files on first consumption).
**Date:** 2026-08-28
...
- [x] **T2.1** Tiled online-softmax attention with knobs: tile shape (Bc, Br), tile dimension order, mantissa width (via T1.1), seq length. G1: in f64 it equals naive attention exactly (golden identity). **DONE 2026-08-29** — `lab_attention` (canonical FA order, direct division); golden identity is TWO-TIER: single-tile configs BIT-IDENTICAL to naive (correction `exp(−inf)=0` inert, same accumulation order), multi-tile pinned at 1.523e-15 rel (exact equality structurally impossible across rescale groups — that non-associativity IS the measured mechanism; recorded in the bench).
- [x] **T2.2** Ordering-law gates: max-diff non-increasing in mantissa bits across ≥2 context lengths; Spearman rank correlation between `R = ⌈S/T⌉−1` ordering and measured max-diff over an (S, T) grid (ordinal gate — the first-order predictor is explicitly not an absolute bound); tile-area ordering reproduced at ≥2 formats; dim-order-swap ordering; square-tile row pinned as the negative control. **DONE 2026-08-29** — all four pinned as tests: mantissa ladder (10 widths × S∈{128,256}) monotone; ρ = 0.8892 over 9 (S,T) points (floor 0.7) + R=15 > R=0 endpoint; tile-area monotone at bits ∈ {10,16}; axis-assignment swap changes deviation at (64,16) and is BIT-IDENTICAL at the square (32,32) — the implementable form of the paper's dim-order knob, honest scope recorded in the bench.
...
- [x] **T3.2** `tol(S)` schedule helper: offline fit `tol(S) = tol(S₀)·f(S/S₀)` from the lab, emitted as a pinned constant table (determinism: hash the fit inputs); two-length probe — a kernel passing tol(S₀) must not flip verdict class at 8×S₀. First consumer: the Issue 753 f16-KV path (80K ctx vs its fixed-shape validation). **DONE 2026-08-29** — `TOL_TABLE_PINNED` (4 rows, deviation × TOL_HEADROOM=2.0 policy) + `band_at` interpolation + `TOL_FIT_HASH_PINNED` (blake3 over the fit config, exact pin) with row VALUES pinned within the ±20% cross-libm band; the two-length probe runs through the Phase-1 `accept` rule: stale S₀ band REJECTS at 8·S₀ (asserted non-vacuous), schedule band ACCEPTS — no class flip.
