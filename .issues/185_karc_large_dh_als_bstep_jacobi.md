...
- [-] **T3 — G1 GOAT gate re-run at K=8, M=8, R=2 (d_h=18_720).** **BLOCKED** — see "Compute-feasibility gap" below and **[Issue 186](186_karc_t3_compute_unblock_deliberation.md)** for the unblock-path deliberation (LAPACK / Householder+QL / parallel Jacobi / gate re-spec). The naive Jacobi eigendecomp of G at d_h=18_720 is ~3.3e13 FLOPs/sweep × ~10 sweeps = ~3.3e14 FLOPs one-time, plus cache-hostile random-access pattern. Even d_h=4_752 (the existing shipping bench) exceeded a 10-minute watchdog timeout in the timing trial — d_h=18_720 is 16× larger. T3 needs a faster symmetric eigensolver — **tracked in Issue 186**, not here. The T1 implementation is correct (T2 proves this); the blocker is purely compute-feasibility.
- [-] **T4 — Memory ceiling check.** **BLOCKED with T3** — same blocker (the example doesn't run yet). Tracked in [Issue 186](186_karc_t3_compute_unblock_deliberation.md). The buffer footprint math is unchanged from the issue body: at K=8, M=8, R=2, r=8, d_h=18_720, the largest single buffer is `Q ∈ R^{18720×18720}` ≈ 2.6 GB f64 (heap-allocated, caller-provided scratch).
- [-] **T5 — Plan 308 promotion decision.** **BLOCKED with T3** — cannot make the promotion call without T3's evidence. Tracked in [Issue 186](186_karc_t3_compute_unblock_deliberation.md). `karc_forecaster` stays opt-in.
