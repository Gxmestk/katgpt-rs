...
- [x] **T3 — G1 GOAT gate re-run at K=8, M=8, R=2 (d_h=18_720).** **DONE 2026-07-21** via Issue 186 §Resolution (Path D3 split-config gate) + Issue 187 T6 (parallel Householder timing trial). The d_h=18_720 measurement ran 2026-07-20 (87 min wall via parallel Householder path; ~22 min via direct Cholesky) and produced the **NRMSE PASS at K=8/M=8/R=2 λ=5e-2 = 9.43e-4**. The threshold leg (≥ 8 LT) was measured separately at K=8/M=24/R=1 λ=5e-3 = 8.16 LT (Phase 1 + Phase 5.3 reproduce). Both legs pass; the compound gate (both in ONE config) is structurally infeasible (NRMSE requires R=2; threshold requires M≥24; R=2 × M=24 → d_h ≥ 166_752 = 222 GB Gram). Full evidence trail in [`.benchmarks/308_karc_goat.md`](../.benchmarks/308_karc_goat.md) Phases 5 → 5.1 → 5.2 → 5.3.
- [x] **T4 — Memory ceiling check.** **DONE 2026-07-21.** The d_h=18_720 timing trial (Issue 187 T6) confirmed the buffer math: largest single buffer `Q ∈ R^{18720×18720}` ≈ 2.6 GB f64 held without blowup on a 64 GB workstation. The T1 implementation's caller-provided-scratch design (no hot-path allocation) is validated at the target config.
- [x] **T5 — Plan 308 promotion decision.** **DONE 2026-07-21.** `karc_forecaster` **PROMOTED TO DEFAULT-ON** under Issue 186 §Resolution Path D3 (split-config gate). See commit `c8324ff2` (Phase 22 addition to the Cargo.toml `default = [...]` list). G2/G3/G4 unchanged: 381 ns/call forecast, 0 allocs, bit-reproducible Wout. Pure modelless (no training). The T1+T2 implementation (large-d_h Jacobi B-step) stays as a useful primitive for future symmetric-eig consumers but is NOT the path the G1 measurement used — full-rank direct Cholesky dominated on both speed and accuracy at d_h=18_720.
