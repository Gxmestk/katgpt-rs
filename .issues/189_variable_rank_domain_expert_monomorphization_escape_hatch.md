...
- [x] **T2 — Implementation.** **DONE 2026-07-22.** Shipped the `variable_rank_router_static!` macro in `crates/katgpt-core/src/variable_rank_domain_expert.rs` (with `#[macro_export]`). Added 3 inherent helper methods on `ClusterHolder<K, L>` (`LATENT_DIM`, `EXPERT_COUNT`, `apply_direct`, `override_pi_direct`) and refactored the `ErasedCluster` impl to delegate (DRY). The macro generates a struct with typed `ClusterHolder<K, L>` fields + a `match`-based dispatch — zero `Box<dyn>`, zero vtable calls. 7 new lib tests (dispatch, scatter-back, override_pi, 10K-input no-NaN, **bit-identical-to-dynamic parity gate**) + 1 new alloc test file (`variable_rank_domain_expert_macro_alloc.rs`) with a 2-phase G4 audit (plain tick + override_pi path, both 0 bytes / 1000 ticks). All 1786 lib tests + 2 alloc tests pass. `--all-features` + `--no-default-features` clean.
- [x] **T3 — G2 re-gate (release).** **DONE 2026-07-22.** Re-ran G2 with the monomorphized router in release mode (warm-up + min-of-5 methodology). Added two bench shapes: macro shared-router (conservative bound) + macro production-shape (per-NPC-owned, tick only). **Result: G2 still FAILS.** Macro shared: ~1.7× baseline (vtable elimination recovered ~25% of overhead vs dynamic ~2.2×). Macro production-shape: ~1.95× (cache thrashing from per-NPC boxed fields — bench artifact). The ~35 ns irreducible overhead (domain gate + projection + scatter + override_pi) over a ~50 ns baseline = 1.7×. The ≤1.0× target is structurally unreachable. Full results + analysis in [`.benchmarks/558_variable_rank_domain_expert_goat.md`](../.benchmarks/558_variable_rank_domain_expert_goat.md) §"Monomorphization re-gate".
- [x] **T4 — Promotion decision.** **DONE 2026-07-22.** G2 still > 1.0× after monomorphization → the variable-rank pattern is fundamentally more expensive per tick. **Feature stays opt-in forever.** The 2.63× entropy gain (G3) is the selling point for diversity-prioritizing consumers, not latency. The monomorphization macro ships as the zero-vtable escape hatch (recovered ~25% of overhead) but is NOT a path to default promotion. No changes to `Cargo.toml` default features.
...
`variable_rank_domain_expert` works (G1/G3/G4/G5 PASS, 2.63× entropy gain) but costs ~2× per-tick latency (G2 FAIL). The monomorphization macro (T2, `variable_rank_router_static!`) eliminated the vtable tax and recovered ~25% of the overhead (dynamic ~2.2× → macro ~1.7×), but G2 still fails — the ~35 ns irreducible overhead (domain gate + projection + scatter + override_pi) over a ~50 ns baseline = 1.7×. **Feature stays opt-in forever.** The 2.63× entropy gain is the selling point, not latency. All 4 tasks (T1-T4) closed.
