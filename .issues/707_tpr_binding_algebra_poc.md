...
**Status:** Phases 1–3 LANDED, GOAT **ALL PASS** (all four T-G8 clauses now carry measured evidence — the intervention battery through a real consumer landed 2026-09-02), feature stays **opt-in**. `katgpt_core::tpr` ships behind `tpr = []` with 29 lib tests + `.benchmarks/698_tpr_binding_algebra_goat.md` (G1/G2/G3/G4/G8 all PASS, three consecutive runs; G8 additionally gates on its corpus being INTERPRETABLE since Issue 711 T4). Promotion is blocked by the **no-default-consumer rule**, not by any gate: T-Promote needs a consumer on its DEFAULT path — two consumers have now landed (riir-clippy 062→Bench 063, measured flat + re-scoped; riir-ai 847 Phase 2→Bench 852, GOAT 10/10 but opt-in), and the call is the consumer owners'. The five `[-]` rows each name a non-engineering unblock (a default-path consumer, a training run, or an owner window). Everything the primitive can prove alone is proven.

**THIRD consumer verdict, 2026-09-03 — riir-clippy Issue 062 T4 closes REFUTED (riir-clippy `.benchmarks/065_t4a_withheld_pair_gate.md`, commit `cd6c068`).** The re-scoped route (b) fuse was gated on a preregistered withheld-pair test against the healer corpus, and it FAILED at its decisive row: in-distribution top-1 climbs **21.1% → 46.8% → 63.3% → 81.1%** across the admissible `d` grid while OOD stays at chance throughout — TPR gets steadily better at identifying pairs it TRAINED on and gains nothing transferable to an unseen `(role, filler)` combination. Margin 0.0 pp against this issue's own 20 pp G8 bar, on a fully readable instrument (63.3% ID, 0 inadmissible refits, null informative).

**This does NOT weaken the primitive's gates and should not be read as doing so.** It is a fact about a corpus, and specifically about the one G8's record already warned it was not evidence for: G8 is a *planted positive control*. Bench 065 is the negative real-corpus companion it named as missing.

**What it does do is corroborate the boundary riir-ai 847 Phase 2 found, from an independent direction.** 847 measured the withheld-`(filler, role)` BINDING axis at 0/2 on the 0.90 bar over quest geometry — construction generalizes novel combinations of *seen* bindings, not novel bindings. Bench 065 measures the same boundary on a real code corpus at 30 trials / 45 scored states / four geometries, with the memorization signature made explicit by the rising ID column. **Two consumers, two domains, two harnesses, same wall** — which is a much stronger statement of the primitive's domain of applicability than either alone, and it belongs in T-Promote's record because it bounds what a future default-path consumer may claim.

**T-Promote's status is unchanged and its arithmetic is worse:** three landed consumers, none default-path, and the third is now refuted rather than merely opt-in.
...
- [x] **T-G8** Withheld-pair accuracy > atomic-dictionary null by a registered margin; intervention battery ≥ registered bar; BoW task-dependence reproduces on a structure-free control family; denoise-readout adopted only where it wins. — **PASS on three of four clauses** (100.0% vs 0.0% at a 20 pp bar with the null verified informative ID; BoW router + shuffled-role control both reproduce on the order-free family; projection adopted only behind its own floor gate). The **intervention-battery clause needed a consumer** — **delivered 2026-09-02 by the second designated consumer**: riir-ai Issue 847 Phase 2 (`quest_tpr`, Bench 852, GOAT 10/10) runs the full battery through the real quest_grammar consumer — role-move surgery additivity **1.788e-7** (latent edit ≡ symbolic-edit-recompose), the acceptance/refusal cascade through the PRODUCTION `GrammarValidator` (sg 12/12, mods 8/8, plural-agreement flips 4/4 correctly refused, OOD-B slot-illegal 2/2), and withheld-pair composition 8/8 vs the 0/8 memorization baseline. All four clauses now have measured evidence.
- [-] **T-Promote** Promote only after ALL gates pass AND a consumer exists (no-default-consumer rule). Demote-loser if a simpler op wins the same slot.
  ...
  **Second consumer LANDED 2026-09-02 — a real GOAT-passing consumer, still not a default-path one.** riir-ai Issue 847 Phase 2 (`quest_tpr` behind opt-in `quest_tpr = ["quest_grammar", "katgpt-core/tpr"]`, Bench 852, commit `c4222f983`) ships construction-mode composition + surgery + the validator cascade over the quest variant world, GOAT 10/10: the OOD-A coverage wall CLOSED (8/8 validator-pass vs 0/8 for every memorization arm), surgery additivity 1.788e-7, planted control 1.0×5, debug ≡ release bit-identical. It also measured the boundary honestly: the withheld-(filler, role) BINDING axis does not extrapolate (0/2 at the 0.90 bar) — construction generalizes novel combinations of seen bindings, not novel bindings (the COGS hedgehog-as-object failure at quest geometry), which bounds the primitive's claim precisely. T-Promote remains **owner-gated**: both consumers are opt-in features, so flipping `tpr` to default would compile dead code into every katgpt-core consumer — the promotion call needs a consumer that wants the primitive on ITS default path (the 847 Promote row is that consumer's owner call, defer-marked there).
