...
- ~~`src/pruners/bomber/players.rs` (2828)~~ ✅ DONE (Issue 175, 2026-07-17): split to `players/` module folder — `mod.rs` (155, well under 2048 ✓) + `helpers.rs` (783, shared utilities) + 7 player-type files (`random_player.rs` 72 + `greedy_player.rs` 125 + `validator_player.rs` 148 + `hl_player.rs` 1016 + `lora_player.rs` 190 + `lora_wasm_player.rs` 237 + `nn_player.rs` 178) + `tests.rs` (102). **Functional** split (not mechanical test extraction) — the file's own comments (`// ── P1: Random ──`, `// ── P2: Greedy ──`, etc.) explicitly delimit each player type, matching the existing pattern in `fft/`, `go/`, `monopoly/` modules. Prior session verdict "CONFIRMED needs functional split" was correct that this is functional — but WRONG that the work was out of scope: the file's own structural comments ARE the natural seam. G1+G3 PASS: 1558/1558 katgpt-core default + 374/374 katgpt-rs under `bomber-wasm` + workspace sweep (1879+ tests, 0 failures); clippy clean workspace-wide. All 6 relevant feature combos compile (default + bomber-wasm + contextual_bandit + binned_blend + kernel_blend + bomber-agent + --all-features).
