...
**Status:** CLOSED — all gates PASS 2026-09-03 (T1–T6 done, T7 deferred; record: `.benchmarks/699_lt2_deep_loop_stability.md`; `lt2_deep_stability` DEFAULT-OFF). T2 baseline verdict: the stable micro fixture is FLAT to T=1024 (argmax 27/27, cos 1.000000, state norm pinned at the fixed point) — the damping knob is insurance, not load-bearing, for this model class; the gate-driven destabilized fixture proves it is real (undamped overflows f32 by τ≈864; damped arms track λ → 1−α+αλ̂ to ≤0.04%).
