...
| f16/bf16 weights | Empirically NOT a win on Apple Silicon (aarch64). Weight-only f16 is **1.7× slower** than f32 (Issue 200, G2 FAIL — f32 activations cap bandwidth reduction at 25%, FCVT latency eats the rest). Full f16 (weights + activations, FHM widening FMA) is only **1.31× faster** — short of the 1.5× promotion gate (Issue 201, Bench 563). Root cause: f32 is already near the bandwidth ceiling (~95–110 GB/s at L3-exceeding sizes) and the dot kernel is not purely bandwidth-bound (FMA throughput + accumulator reduction eat the theoretical gain). f32 stays the production dtype for the **f32 forward path**. The `simd_dot_f16_f32` / `matmul_f16` kernels exist as opt-in reference paths (`katgpt-forward/f16_weight_quantization` feature) but are G2-failed and not promoted to default. **NOTE (Issue 206/207, 2026-07-31): the INT8+INT8 path (originally filed as a non-goal here) is now a GOAT + DEFAULT-ON in `katgpt-moka-wasm`** — the bandwidth-ceiling argument applied to the `katgpt-core` f32 substrate but NOT to the Moka WASM forward path (where `i8x16.dot_s` / SDOT use a different execution unit than f32 FMA, weight footprint drops to L1, and V8 JIT lacks f32 FMA). See [Bench 565](../../.benchmarks/565_int8_int8_sdot_positive.md) (Issue 207 removed per noise-reduction rule; see commit 7da5cf76). The f16 non-goal stands; the INT8 non-goal was correctly overturned by measurement. |
