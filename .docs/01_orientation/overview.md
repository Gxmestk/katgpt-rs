...
| `karc_forecaster` | `katgpt-core/karc_forecaster` | KARC — Kolmogorov-Arnold Reservoir Computing delay-basis ridge trajectory forecaster (Plan 308, Research 288, arXiv:2606.19984 Huang/Kurths/Tang). `KarcForecaster<D,M,K>` + sealed `KarcBasis` trait (Fourier/Chebyshev/BSpline) × closed-form ridge readout. Phase 2 ships higher-order R=2 + chunked Gram + ALS low-rank `Wout ≈ A·B` (the form that persists into a `KarcShard` in riir-neuron-db). **Opt-in** — Phase 5.1 (2026-07-20): G1 NRMSE **9.43e-4** PASS at K=8/M=8/R=2 d_h=18_720 (λ=5e-2 λ-sweep recovering the underdetermined system); G1 threshold 7.23 LT ❌ (10% short — capacity/delay problem, not regularization; threshold flat across λ). G2 (381 ns), G3 (0 allocs), G4 (bit-reproducible Wout) ALL PASS. **Compute blocker resolved**: d_h=18_720 full-rank direct Cholesky ~29 min wall (was projected 6 h via Jacobi). Promotion deferred on threshold leg — 3 open paths (K=10 extrapolation predicts ~8.5 LT; Issue 186 Path D gate re-spec; N=20_000+ training data). See [`.benchmarks/308_karc_goat.md`](../../.benchmarks/308_karc_goat.md). |
