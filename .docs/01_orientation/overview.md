...
| `flashmemory_sparse` | `katgpt-attn/flashmemory_sparse` (implies `mla_attention` + `dash_attn`) | FlashMemory periodic sigmoid-threshold sparse attention for MLA (Issue 584, Research 436, arXiv:2606.09079). τ-step refresh + `sigmoid ≥ 0.5` block selection + centroids from `MlaKVCache` latent KV, on the VortexFlow substrate. **Opt-in** — G1 PASS on real Kimi-K3-0.40B weights (cos ≥ 0.9566, ~74% KV reduction, bench_021), G3 PASS (169 tests), G4 PASS (0 allocs, bench_022), G5-M3 ~74% at ≤8K with G1 holding at all scales (bench_024); G2 + 90%@256K BLOCKED on 4090 (Phase 2). Honest NIAH negative: single-layer retrieval not testable (bench_023). Zero runtime cost unless invoked. |
