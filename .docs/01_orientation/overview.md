...
Default features: **72** in katgpt-core (`crates/katgpt-core/Cargo.toml`) + **133** in the root crate (`Cargo.toml`) = **183 unique** features. The authoritative source for the exact list is the `default = [...]` array in each `Cargo.toml` (hand-maintaining a prose enumeration of 183+ features proved unmaintainable — at the time of replacement the list was missing 41 katgpt-core defaults + 61 root defaults, and had 21 stale entries including function names that were never feature flags). See `opt_in_features.md` §69 for the DEFAULT-ON cross-reference of substrate/utility primitives not documented elsewhere in the catalog. All defaults are GOAT-proved modelless primitives (Plans 051–571). Production best perf + accuracy.
