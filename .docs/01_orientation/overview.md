...
| `swir_switch_thinking` | `thinking_cot` | SwiR Switch-Thinking — explicit↔latent reasoning mode controller driven by entropy trends, asymmetric dwell windows + switch-count overthinking guard (Plan 275, Research 241, **opt-in**: synthetic GOAT G3–G8 + G1c/G2p pass; real-model G2 = **1.37× PASS** at `w_e_to_l=32, c_max=64` (riir-ai Plan 313, n=5); G1 = 0% blocked by Gemma 2 2B capability — needs Qwen3-4B/8B; promotion pending n=20+ confirmation) |
...
| `depth_invariance` | `katgpt-core/depth_invariance` | Depth-Invariance Diagnostic + MagnitudeRegularizedResidual — root-cause counterpart to BeliefRankPruner / GainCostLoopHalter / latent_functor/reestimation / micro_belief/coherence_bench (Plan 306, Research 286, arxiv 2605.09992). Detects DepthSpecificRefinement / Collapsed / DepthInvariant on flattened `&[f32]` state chains. **Opt-in**: G1 (8 correctness tests) + G2 (paper finding reproduced on random-init BeliefDrafter) + G3 (negative control on AttractorKernel + positive control on unclamped leaky) + G4 (latency, aspirational) all pass; promotion to default is a deliberate parent decision pending. |
...
| `triggered_injection` | — | TriggeredInjectionGate — sigmoid-thresholded inject/skip hot-path gate (Plan 278, Research 244, opt-in) |
| `manifold_power_iter_router` | — | Manifold Power Iteration MoE Router — one-shot router-row conditioning at snapshot swap via shared `spectral_retract` helper (Plan 279, Research 246, arxiv 2606.12397, **opt-in**: GOAT 11/11 green, G1 λ-alignment + G2 MaxVio reduction + G3 zero per-token overhead) |
...
| `subspace_phase_gate` | `katgpt-core/subspace_phase_gate` | Participation ratio + numerical rank + N≥d phase-transition gate + runtime Jacobian SVD (Plan 301, Research 279, arxiv 2409.02426). Pure numeric substrate; consumed by Plan 312. **Opt-in** — Phase 2 G1 PASS; Phases 3–5 deferred. |
...
| `qgf_oracle` | — | `QGradientOracle` trait — drop-Jacobian critic gradient `∇_a Q(s, â_1)` (Plan 268 F3). Plus `NoGuidanceOracle` (freeze-tier no-op, zero confidence). |
