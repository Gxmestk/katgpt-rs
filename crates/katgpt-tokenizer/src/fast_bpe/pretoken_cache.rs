...
// Upstream file: `src/bpe/pretoken_cache.rs`.
// Upstream commit at vendor time: master 2026-07-25.
//
// Adaptations vs upstream:
// - Dropped the `super::madvise_hugepage` Linux-only hint (would force a
//   `libc` dep on this leaf-clean crate). The hint is a no-op off Linux
//   anyway; on Linux the table still works (just walks the dTLB on every
//   probe until khugepaged collapses the mapping — perf regression only,
//   no correctness impact). When this crate eventually grows a Linux-only
//   perf-tuning surface, restore the hint there.
// - `ProbeView::probe_pair`'s aarch64/x86_64 inline-asm `csel`/`cmovne`
//   optimizations kept verbatim — they're the load-fusion win.
//
...
///   chunk ahead, [`ProbeView::prefetch`] a few probes ahead) instead of
///   stalling on it.
...
///   extension word (see upstream `tiktoken::pack_val_inline`) holding up
///   to four tokens inline — one dependent load, and no second random
///   access into the token arena.
/// - At ~64 MB the table also blows the dTLB through 4 KiB pages, so the
///   backing memory is 2 MiB-aligned (the `Layout` requests it; without
///   `madvise_hugepage` the OS may still serve 4 KiB pages until khugepaged
///   collapses them).
///
/// Linear probing over aligned pairs: a bucket is slots `idx` and `idx + 1`
/// with `idx` even, so both share one 64 B line, and [`ProbeView::probe_pair`]
...
/// since `EMPTY_KEY` is 0), 2 MiB-aligned allocation. A plain `Box<[Entry]>`
/// can neither over-align nor keep dealloc's layout in sync with an
/// over-aligned alloc.
struct Slots {
    ptr: NonNull<Entry>,
    cap: usize,
...
impl Slots {
    ...
    fn new_zeroed(cap: usize) -> Self {
        ...
        // NOTE: upstream calls `super::madvise_hugepage(raw, layout.size())`
        // here BEFORE first touch. We omit that hint to stay leaf-clean
        // (no `libc` dep). The zeroing write below faults the mapping in
        // as 4 KiB pages on Linux; khugepaged will collapse it to 2 MiB
        // pages asynchronously. Perf regression on Linux only; correctness
...
impl ShortPretokenCache {
    ...
    /// 3/4-load threshold as [`Self::insert`], with a 2^16-slot floor),
    /// starting from at least `min_slots` slots.
    #[allow(dead_code)]
    ...
    /// Request the probe's cache line into L2 only. The encode loop would
    /// call this a full chunk (hundreds of cycles) before the probe — enough
    /// to cover DRAM — without evicting the span walker's L1 working set the
    ...
    /// the first empty slot of the walk. [`Self::insert_at`] then skips
    ...
    fn grow(&mut self) {
        ...
        for i in 0..old.cap {
            ...
            let idx = self.first_empty(pretoken_key_hash(e.key));
            // SAFETY: first_empty returns an in-bounds index.
...
impl ProbeView {
    ...
    pub(crate) fn probe_pair(&self, key: u128, h: u64) -> (u64, u64, bool) {
        ...
        let (val, ext) = {
            ...
            // upstream `profiling/x86_port_plan.md` §1.1.
