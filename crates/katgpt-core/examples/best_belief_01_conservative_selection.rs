...
fn section_5_lut_vs_closed_form() {
    ...
    println!("  In-LUT (S=10, F=5, ε=0.05):    BB = {:.8}", in_lut);

    // Out-of-LUT case: large S (falls back to closed form).
    let cold_path_large = best_belief_score(100, 50, 0.05);
    println!("  Cold path (S=100, F=50, ε=0.05): BB = {:.8}  (S ≥ 32 → closed form)", cold_path_large);

    // Out-of-LUT case: non-standard ε (falls back to closed form).
    let cold_path_eps = best_belief_score(10, 5, 0.07);
    println!("  Cold path (S=10, F=5, ε=0.07):  BB = {:.8}  (non-standard ε → closed form)", cold_path_eps);

    // Edge: uniform prior.
    let uniform = best_belief_score(0, 0, 0.05);
    println!("  Uniform prior (S=0, F=0, ε=0.05): BB = {:.8}  (= ε by definition)", uniform);
    println!();
