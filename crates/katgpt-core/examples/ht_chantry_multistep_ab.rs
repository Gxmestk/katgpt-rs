...
use katgpt_core::multi_agent_path::{
    BlockingCount, EscalationBudget, GridFlowField, GridMap, GridPos, GuidanceConfig, JointConfig,
    LifelongLaCam, SpaceTimeGuidance, WarmStartCache, WarmStartScheme,
};
...
fn run_with_budget(map: &GridMap, budget_name: &str, multistep: bool) -> (f64, f64, usize) {
...
fn main() {
    ...
    let map = katgpt_core::multi_agent_path::GridMap::from_movingai(include_str!(
        "../benches/data/ht_chantry.map"
    ))
    .unwrap_or_else(|| {
    ...
    let npassable: usize = (0..map.height)
        ...
        .count();
    println!(
        "Map: ht_chantry-real ({}×{} = {} cells, {} passable = {:.1}% open)\n",
        map.width,
        map.height,
    ...
    let (t_multistep, m_multistep, c_multistep) =
        run_with_budget(&map, "multistep (Issue 546)", true);

    println!();
    println!("─── Delta ───");
    let delta_t = t_multistep - t_default;
    let delta_pct = if t_default > 0.0 {
        delta_t / t_default * 100.0
    } else {
        0.0
    };
    ...
    println!(
        "  Median tick:  {delta_latency:+.2}ms (multistep is {}expensive)",
