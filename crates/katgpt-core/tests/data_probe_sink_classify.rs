...
use katgpt_core::data_probe::{
    CachedSinkClassification, SinkAwarePolicy, SinkClassifierConfig, SinkKind, StableRankScratch,
    apply_dual_policy_gate, apply_dual_policy_gate_cached, apply_dual_policy_gate_cached_flat,
    apply_dual_policy_gate_flat, classify_all_sinks, classify_all_sinks_flat, classify_sink_at,
    classify_sink_at_flat, stable_rank_update_into, stable_rank_update_into_flat,
};
...
fn issue001_cached_invalidate_forces_reaudit() {
    ...
    // First call: audit.
    let _ =
        apply_dual_policy_gate_cached(&attn, &values, &o, 2.0, &mut scratch, &mut cached, &mut out);
...
fn plan288_classify_all_sinks_flat_parity() {
    let n = 5;
    let d = 4;
    let mut values_rows: Vec<Vec<f32>> = (0..n).map(|i| vec![1.0 + 0.1 * i as f32; d]).collect();
