...
fn main() {
    ...
    println!(
        "=== Plan 562: event_log_query demo ({} events) ===\n",
        log.len()
    );
    ...
    println!("  ... ({total_actions} total actions)\n");
    ...
    let back_half_rewards = log.count_where(
        &Predicate::event_type(EventType::RewardSignal).and(Predicate::id_range_from(EventId(50))),
    );
    println!("  {back_half_rewards} reward signals in the back half (id >= 50)\n");
    ...
    if let Some(first_eval) = log.first_where(&Predicate::event_type(EventType::Evaluation)) {
        println!(
            "  first evaluation: id={} score={:.3}",
            first_eval.id.0, first_eval.payload.score
        );
    }
    if let Some(last_action) = log.last_where(&Predicate::event_type(EventType::Action)) {
        println!(
            "  last action:      id={} score={:.3}",
            last_action.id.0, last_action.payload.score
        );
    }
    println!();
    ...
    println!("\n  Composed (high-score AND id < 50): {composed_count} events");
