//! SDAR-gated FFT Player — sigmoid-gated reward FFT Tactics arena player.
//!
//! Structurally identical to RubricFFTPlayer but replaces rubric-vector components
//! with SDAR sigmoid-gated scalar components. Uses [`SdarBanditPruner`] for
//! reward gating and [`SdarGatedAbsorbCompress`] for promotion gating.
//!
//! # Architecture
//!
//! ```text
//! SdarFFTPlayer
//!   ├── FFTTemplateProposer        (UCB1 over 10 templates — same as GZeroFFT/RubricFFT)
...
//! 4. Compute scalar δ for proposer + SDAR reward from battle state
//! 5. Feed δ to proposer, SDAR reward to bandit/absorb
...
use crate::sdar::{SdarAbsorbConfig, SdarBanditPruner, SdarGatedAbsorbCompress};
...
/// SDAR sigmoid-gated FFT player.
///
/// Replaces RubricFFTPlayer's rubric-vector reward with scalar SDAR-gated
/// reward signals. Uses [`SdarBanditPruner`] for asymmetric trust gating
/// on bandit updates and [`SdarGatedAbsorbCompress`] for sigmoid-gated
/// knowledge promotion.
pub struct SdarFFTPlayer {
    /// UCB1 proposer for strategy templates.
    template_proposer: FFTTemplateProposer,
    /// SDAR sigmoid-gated bandit pruner.
    sdar_bandit: SdarBanditPruner<NoScreeningPruner>,
    /// SDAR sigmoid-gated absorb-compress for knowledge promotion.
    sdar_absorb: SdarGatedAbsorbCompress<NoScreeningPruner>,
    /// Per-round action metrics for reward scoring.
...
impl SdarFFTPlayer {
    /// Create a new SDAR-gated FFT player with given ID.
    pub fn new(id: u8) -> Self {
        let inner_bandit =
            BanditPruner::new(NoScreeningPruner, BanditStrategy::Ucb1, NUM_TEMPLATES);
        let sdar_bandit = SdarBanditPruner::new(inner_bandit, NUM_TEMPLATES);

        let absorb_layer =
            AbsorbCompressLayer::new(NoScreeningPruner, NUM_TEMPLATES, CompressConfig::default());
        let sdar_absorb =
            SdarGatedAbsorbCompress::new(absorb_layer, NUM_TEMPLATES, SdarAbsorbConfig::default());
    ...
    /// Also feeds outcome reward to SDAR bandit/absorb.
    pub fn update_outcome(&mut self, survived: bool, kills: u32, damage: i32, healing: i32) {
        let outcome_reward = if survived { 1.0 } else { -2.0 }
            + kills as f32 * 0.5
            + damage as f32 * 0.01
        ...
        // Feed outcome reward to SDAR bandit/absorb
        self.sdar_bandit.update(0, outcome_reward);
        let q_val = self.sdar_bandit.q_values()[0];
        self.sdar_absorb.observe_with_q(0, outcome_reward, q_val);
    ...
    /// Compute scalar SDAR reward from battle state and round stats.
    ///
    /// Collapses the same multi-criteria as `compute_fft_rubric` into a
    /// single weighted f32 using the weights from `RubricTemplate::fft_tactics()`.
    fn compute_sdar_reward(unit: &Unit, _state: &BattleState, round_stats: &RoundStats) -> f32 {
        ...
        // Weighted blend — same weights as RubricTemplate::fft_tactics()
        task_fulfillment * 0.5 + completeness * 0.15 + constraint_satisfaction * 0.35
    }
...
impl FftPlayer for SdarFFTPlayer {
    fn select_action(&mut self, unit_id: u8, state: &BattleState, rng: &mut Rng) -> Action {
        ...
        // 5. Compute scalar SDAR reward from battle state
        let reward = Self::compute_sdar_reward(unit, state, &self.round_stats);

        // 6. Feed δ to proposer (same as GZeroFFT/RubricFFT)
        self.template_proposer.observe_delta(template_id, delta);

        // 7. Feed SDAR reward to bandit
        self.sdar_bandit.update(template_id, reward);

        // 8. Feed SDAR reward to absorb with Q-value
        let q_val = self.sdar_bandit.q_values()[template_id];
        self.sdar_absorb.observe_with_q(template_id, reward, q_val);

        // 9. Select action via blended scores + safety filter + ε-greedy
        let action_type = self.select_best_action(&hinted_scores, unit_id, state, rng);

        // 10. Record action with δ and update round stats
        ...
        // 12. Movement: move toward target if out of range
...
impl Default for SdarFFTPlayer {
    fn default() -> Self {
        Self::new(0)
    }
}
...
mod tests {
    ...
    #[test]
    fn test_compute_sdar_reward_knight_attacking() {
        ...
        let reward = SdarFFTPlayer::compute_sdar_reward(&unit, &state, &round_stats);
        // Knight with 3 attacks → task_fulfillment = 1.0
        // completeness = 0/3 = 0.0
        // constraint_satisfaction = hp_pct (alive, full HP)
        // reward = 1.0*0.5 + 0.0*0.15 + 1.0*0.35 = 0.85
        assert!(
            reward > 0.5,
            "Reward should be > 0.5 for knight with full attacks, got {reward}"
        );
    }

    #[test]
    fn test_compute_sdar_reward_healer() {
        ...
        let reward = SdarFFTPlayer::compute_sdar_reward(&unit, &state, &round_stats);
        // WhiteMage with 2 heals → task_fulfillment = 1.0
        // completeness = min(3/3, 1.0) = 1.0
        // reward = 1.0*0.5 + 1.0*0.15 + hp_pct*0.35
        assert!(
            reward > 0.65,
            "Reward should be > 0.65 for healer with full heals, got {reward}"
        );
    }
    ...
    fn test_compute_sdar_reward_dead() {
        ...
        let reward = SdarFFTPlayer::compute_sdar_reward(&unit, &state, &round_stats);
        // Dead unit → constraint_satisfaction = 0.0
        // reward = 1.0*0.5 + 0.0*0.15 + 0.0*0.35 = 0.5
        assert!(
            (reward - 0.5).abs() < 1e-6,
            "Reward should be 0.5 when dead (task_fulfillment only), got {reward}"
        );
    }

    #[test]
    fn test_new_player_initial_state() {
        let player = SdarFFTPlayer::new(3);
    ...
    fn test_select_action_returns_valid() {
        let mut player = SdarFFTPlayer::new(0);
        let state = BattleState::new();
        let mut rng = Rng::new();
    ...
    #[test]
    fn test_default_creates_player() {
        let player = SdarFFTPlayer::default();
