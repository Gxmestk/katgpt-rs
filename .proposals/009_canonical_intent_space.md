...
**Status:** Proposal — **P2 RAN 2026-07-26, G5 FAILED (Bench 422) for square Procrustes; P1 RAN 2026-07-26, G5 PASSED (Bench 423) for joint-SVD SubspaceAdapter at k ∈ {2,4}.** Cross-arch path **restored** modellessly; Super-GOAT candidate **re-promoted** from "GOAT (intra-arch only)" to "Super-GOAT candidate (cross-arch viable)", conditional on still-unrun G6. See [riir-train/.benchmarks/423_canon_p1_subspace_validation.md](../../riir-train/.benchmarks/423_canon_p1_subspace_validation.md). Plan remains unopened pending P3 (G6 generation-quality gate).
...
- [ ] T4.1 Construct `d_Rust_idiom` canonical direction (from Rust Reference + clippy lint corpus, deterministically — modelless construction per AC-Prefix G1 lesson)
- [ ] T4.2 G6: steer Gemma and Llama via the same canonical direction; measure Rust eval delta vs sysprompt floor
- [ ] T4.3 If G6 fails → keep as intra-arch GOAT, narrow the selling point, demote verdict
