...
A **GOAT-proved** neuro-symbolic micro-Transformer with speculative decoding, constraint pruning, and **574 feature flags (197 default-on, all GOAT-proved)** — built in Rust. Pure algorithms, zero side effects, MIT licensed.
...
The default production stack has **197 GOAT-proved default-on features** (574 total flags), but they don't all run on every token. The architecture uses **layered gating** — most features are bandit-driven, Option-gated, or compile-time-only.
...
📖 **Full feature flag table (574 flags):** [`.docs/09_feature_catalog/opt_in_features.md`](.docs/09_feature_catalog/opt_in_features.md) and [`Cargo.toml`](Cargo.toml).
