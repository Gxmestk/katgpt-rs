...
| **Fourier Flow** (`flow_field_nav`) | FFT-smoothed shared flow fields for O(1) crowd navigation (Plan 242) | GOAT PASS 46.9%, opt-in. **Dual-LEO fusion paths** (Plan 459 + Plan 460): pre-max Q-slice α-mix demoted (G5 FAIL — max-pool washes out the mix); post-max potential α-mix **PROMOTED as the recommended dual path** (G5' PASS at α=0.10: 31.5% stuck-NPC reduction). Doc-level promotion only — `flow_field_nav` + `dual_leo` were already default-on; the dual path is a sibling API (`get_or_compute_dual_postmax`). |
...
- [QGF DualLeoOracle — 3rd `QGradientOracle` impl (G5 FAIL synthetic, opt-in)](.plans/467_qgf_dual_leo_oracle.md)
