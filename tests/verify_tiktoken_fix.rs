...
fn verify_real_tokenizer_token_count() {
    ...
    let data = match std::fs::read(model_path) {
        Ok(d) => d,
        Err(e) => {
            eprintln!("SKIP: {} not found ({})", model_path, e);
            return;
        }
    };
    ...
    eprintln!("\nTotal: {} tokens across {} examples (avg {:.0}/ex)", total, tests.len(), total as f64 / tests.len() as f64);
    ...
    assert!(
        avg < 400.0,
        "avg token count {} should be < 400 (close to Python's ~297); got {}",
        avg,
        avg
    );
}
...
fn verify_exact_token_ids_match_python() {
    ...
    let cases: &[(&str, &[usize])] = &[
        ...
        ("pub struct Foo<T> { x: T, y: Vec<u8> }",
         &[13241, 2901, 52361, 6443, 29, 384, 1288, 25, 377, 11, 364, 25, 20431, 52794, 23, 29, 457]),
    ];
