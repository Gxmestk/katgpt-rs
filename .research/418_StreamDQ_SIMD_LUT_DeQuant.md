...
> **PASS-Redirects (synthesis):** Li et al. [arXiv:2512.06443 "Vec-LUT: Vector Table Lookup for Parallel Ultra-Low-Bit LLM Inference on Edge Devices"] (MobiSys 2026, code in vlut.cpp/llama.cpp) — PASS. Vec-LUT fixes the scalar-LUT-per-token redundancy in **parallel (multi-token) CPU inference** of 1.58-bit LLMs by widening LUT entries to per-token vectors (one 1→N lookup per index) + a vector-LUT-centric tensor layout + cache-aware streamed lookup (4.2× on edge CPUs). The regime is not a stack workload — CPU ternary here is the tiny sequential GEMV drafter (decode = 1 token by construction, the token axis doesn't arise), parallel/prefill inference runs on GPU arithmetic paths (dp4a/rowtiled, compute-bound, no LUT), and the one CPU LUT we do ship (`simd_lut_dequant`, this note) is **weight-keyed — token-invariant by construction**, immune to the per-token rebuild/access redundancy Vec-LUT eliminates (a problem only activation-keyed T-MAC-style tables have). Where we DO run activation-keyed LUTs (`gemv_ternary_plane_lut`, GPU GEMV), the table is threadgroup-resident and row-amortized. Family survey complete: T-MAC/BitNet.cpp (memory LUTs) → T-SAR [arXiv:2511.13676, hardware in-register] → Vec-LUT (software vector layout) all target the edge-CPU 1.58-bit regime; our designs already sit on the on-chip/token-invariant side of each axis. No new primitive.
