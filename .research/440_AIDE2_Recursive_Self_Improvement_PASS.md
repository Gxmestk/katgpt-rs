...
> **PASS-Redirects (synthesis):** Kim et al. [arXiv:2608.24735 "Meta^n: Recursive Self-Improvement through Emergent Depth"] — the strongest empirical descendant of the AIDE² class: fixed Ω + recursion on growing input beats the frozen-driver family (Gödel Agent 0.628 @10× budget vs 0.870; ARC-AGI-2 0.331 vs ≤0.054); verdict unchanged (code-level RSI, no modelless substrate for Ω) — see Research 512.
> **PASS-Redirects (synthesis):** Ning, Tieu, Fu, Wei et al. [arXiv:2605.18747 "Code as Agent Harness: Toward Executable, Verifiable, and Stateful Agent Systems"] (UIUC+Meta+Stanford, 102-page survey, 2026-05) — the field-level taxonomy of this note's entire territory: its §3.5 "Agentic Harness Engineering" (deep telemetry → Evolution Agent → governed harness mutation) and its un-distilled citations AutoHarness [arXiv:2603.03329] / Meta-Harness [arXiv:2603.28052] / AHE [arXiv:2604.25850] are all the AIDE² LLM-dependent class (the *proposal* mechanism is an LLM reading failure traces and generating harness edits). The modelless instance of §3.5 already ships as riir-clippy `self_evolve` (TrajStore deep telemetry + LatentSkillEvolution + EvidenceTier governed mutation + score-bench regression floors); its §3.4 PEV loop ships as `fix_verify`; its §5.2.3 "self-evolution without regression" open problem is answered by the Issue-037 E2E gates. PASS — survey validates, reveals no gap; see [riir-clippy/.research/001](../../riir-clippy/.research/001_self_evolving_latent_first_agent.md) for the shipped instance.
