...
> **PASS-Redirects (synthesis):** Kim, Jin & Kim [arXiv:2608.14333 "Beyond Capacity: Scalable MoE LLM Inference via High-Bandwidth Flash with Direct GPU and HBM Paths"] — the expert-prefetch family's EXACT member: for bias-free scale-invariant MoE routers, logits P = RMSNorm(X + O·W_out)·W_r decompose as α·(P_base + P_attn) with P_base = X·diag(γ)·W_r computable BEFORE attention and P_attn = O·(W_out·diag(γ)·W_r) right after the attention output; α > 0 is rank-invariant, so top-k is exact with no prediction — removing the accuracy question D1's temporal-correlation metric exists to answer. Scoped out of our stack twice over: Kimi-K3's noaux_tc router (sigmoid + per-expert additive `e_score_correction_bias`) is the paper's own excluded class (bias-after-link makes ranking α-dependent), and our experts are device-resident with no offload tier to prefetch from.