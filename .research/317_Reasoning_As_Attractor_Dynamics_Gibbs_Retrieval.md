...
|---|---|---|
| Trajectory energy / spectral entropy | `spectral_entropy_dct`, `spectral_entropy_dct_into` (Type-II DCT, per-token, [0,1], zero-alloc) | `katgpt-rs/crates/katgpt-attn/src/chiaroscuro/entropy.rs` (Plan 269) |
...
| Gibbs weight `exp(-β·H)` | `boltzmann_probabilities`, `boltzmann_sample_batch` (τ-controlled softmax); PlackettLuce Gibbs sampling (I=1000, B=200) | `crates/katgpt-pruners/src/opus/boltzmann.rs` (Plan 129, B039) |
| Inverse-energy trajectory scorer | `exp(-λ_c · T)` cost-penalty scorer | `benches/latent_thought_flow_scorer_bench.rs` (R263) |
| Per-answer basin aggregation | `majority_vote`, `rank_by_consistency`, `select_best_variant`, `ConvergenceSelector::MajorityVote` | `crates/katgpt-speculative/src/parallel_probe.rs`, `crates/katgpt-speculative/src/ppot/rank.rs`, `types.rs` |
