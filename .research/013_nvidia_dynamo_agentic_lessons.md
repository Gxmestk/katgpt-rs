...
- [x] Add `/v1/tokenize` endpoint to REST module for pre-request token counting — `tokenize_handler` in anyrag `gist/anyrag/crates/server/src/handlers/catalog.rs`
...
- [x] Add `/v1/models/{domain}` metadata endpoint to anyrag — `get_domain_model_handler` in `gist/anyrag/crates/server/src/handlers/catalog.rs`
