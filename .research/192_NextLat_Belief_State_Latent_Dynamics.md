...
> **PASS-Redirects (synthesis, 2026-08-25):** Hao et al. [arXiv:2412.06769 "Training Large Language Models to Reason in a Continuous Latent Space"] (Coconut — the paper this note's continuous-thought lineage descends from) — **PASS** per survey 325 §7.1 (routed here; do-not-re-distill held). Re-examined via adversarial panel: v4 is cosmetic over v3 (no new mechanism); the latent-representation core stays GD-dependent (untrained feedback ≈ no-CoT, 14.4 vs 34.1 on GSM8k); only the §4.4 height-confidence measurement + Fig. 6 gap-narrowing escaped as katgpt-rs Issue 688 (commit-timing on `mux`); training-family rows batched into riir-train Plan 352.
