...
> **Related Research (Super-GOAT sibling):** 466 (CP^(d-1) Symmetric-Space Hopfield — this note is the WRITE-side cousin on R^r; 466 is the RECALL-side Super-GOAT on CP^(d-1) with BBP-protected top-eigenvector recall and α_c ∝ d² capacity scaling. Together they cover online-write + offline-recall with modelless associative memory.)
...
> **PASS-Redirects (synthesis):** Ding [arXiv:2607.17560 "Reinforcement Learning: From Algorithms To Foundation Models"] (Princeton PhD thesis 2026) — Part 2's 'long-horizon memory architectures' (VRAG retrieval-augmented video world model w/ global state + long-context window; Recurrent Autoregressive Diffusion 'global memory meets local attention'; Mamba + TTT) are all training-heavy world-model architectures, BUT the modelless memory patterns they distill to are already shipped: δ-Mem associative state (this note) IS the 'global memory' half; Plan 430 GDN+HOLA dual-path ships 'global recurrent state + local attention cache' as an inference primitive; Mirage latent spatial memory (R245 → `SpatialMemory` in riir-games-civ) covers VRAG's retrieval-augmented latent read; n-iter TTT loop → Sleep Consolidation N-pass (Plan 154, per R408 PKM table). The thesis's video-generation training → riir-train.
