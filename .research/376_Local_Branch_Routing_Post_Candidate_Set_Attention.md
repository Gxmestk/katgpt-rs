...
| **Code** | `crates/katgpt-core/src/branching/router.rs::BranchRouter` (Plan 329) | Dot-product snap + Jaccard fallback routing among cognitive branches; ~301ns hot path on 64-branch bank D=8. | Routes among **memory branches** (cognitive domain), not **token candidates** (decode domain). Same routing arithmetic, different operand type. |
...
- **Architectural cousin**: ColliderPruner (CCCP) — `katgpt-rs/crates/katgpt-band/src/collider_pruner.rs`, Plan 265, Research 232.
