...
| "gain/cost halting" (cross-paper) | `GainCostLoopHalter`, "coherence-decay re-estimation trigger" | Research 282, `riir-ai/crates/riir-engine/src/latent_functor/reestimation/mod.rs` |
...
| Paper × riir-ai/crates/riir-engine/src/latent_functor/reestimation/mod.rs: "re-estimation trigger fires when an NPC's HLA state fails to converge under its current functor." | The reestimation trigger already fires on coherence decay (`coherence < tau_reest`) — which is functionally the "FP not reached" signal. Adding a FP-iteration count as an additional trigger is a parameterization, not a new mechanism. | Gain at most. |
