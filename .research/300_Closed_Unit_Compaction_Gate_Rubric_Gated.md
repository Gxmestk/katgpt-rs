...
>
> **PASS-Redirects (synthesis, 2026-08-07):** Chen, Lin, Wang [arXiv:2607.28457 "SVR: Self-Verifying Refinement via Joint Verdict-Confidence RL"] — PASS on the modelless side. SVR inference-time stopping rule is architecturally covered by CUCG CompactionDecision::Compress / Continue / Forced (this note 3-way commit/refine/forced decision) + EarlyStopGate<P, confidence_threshold> (the gamma=0.85 threshold analog). SVR value is its TRAINING recipe (multi-turn GRPO with Brier calibration + asymmetric overconfidence penalty + stop-readiness reward) — all four reward components require ground-truth y, unavailable modellessly at runtime. Path 0 decomposition: inference rule decomposes (shipped), training objective does NOT. Per Path 0.5 (training-applicable, model-based track via Plan 318) -> Plan in riir-train/.plans/328. The PSE (Premature Stop Error) metric + asymmetric overconfidence penalty are useful diagnostic axes for future CUCG benchmark iterations but are not runtime-actionable without y.
