...
> **PASS-Redirects (synthesis, 2026-08-11):** Wei et al. [arXiv:2601.12538] (UIUC/Meta/Amazon/Google DeepMind/UCSD/Yale, Jan 2026) — 135-page survey of the full agentic-reasoning taxonomy (foundational planning/tool/search, self-evolving feedback/memory, collective multi-agent roles/collaboration/training, applications, benchmarks, open problems). PASS: every constitutive property the survey requires for an agentic substrate already ships — riir-agents (latent-space task reasoning + Sheaf-ADMM multi-agent consensus, Plan 523), riir-rag (retrieval), NeuronShard+Raven/delta-Mem+MAPE-K (memory), MerkleFrozenEnvelope+LoraPair Plan 025 (parametric adaptation), riir-wasm LEAP/MUSE (validator-driven feedback), NPC Mind Reading guide (riir-ai/.research/133, ToM). Survey section 8.5 Latent Agentic Reasoning open problem is exactly our default operating mode (Research 411 CoT-vs-Latent covers the formal comparison). Survey section 4.2.3 Post-training Memory Control (Memory-R1, Mem-alpha, MemAgent) is a training-method CLASS — would route to riir-train per the research-skill section 3.5 Path 0.5 only if a specific recipe is picked, not actionable from this survey alone. No new mechanism, no actionable gap. Closest shipped cousin: this note (380) + private guide riir-ai/.research/312.
>
> **Application-level relevance update (2026-08-12):** the survey's verdict is STILL PASS at the mechanism level (taxonomy, not a primitive). It gains **application-level relevance** under [Proposal 035](../../riir-ai/.proposals/035_persistent_research_automation_agent.md) (persistent research automation agent): the survey's abstract explicitly names "autonomous research" as a covered application domain ("science, robotics, healthcare, autonomous research, and mathematics"), and its three-dimension taxonomy (foundational / self-evolving / collective) maps 1:1 onto Proposal 035's architecture (riir-agents TaskGraph / Plan 529 shared context + ExperienceGraph / DeLM shared verified context Research 337). The survey is now a **positioning reference** for Proposal 035 — it validates the use case externally + provides the canonical vocabulary. No new mechanism, no re-distillation; the cross-reference is the entirety of the update.
> **PASS-Redirects (synthesis, prior):** Scofield [arXiv:2601.15077 "Multi-Agent Constraint Factorization Reveals Latent Invariant Solution Structure"] — operator-theoretic proof that MAS = composition of constraint-enforcement operators converges to intersection invariant. Maps onto Research 438 (Sheaf-ADMM, the shipped instance) + katgpt-rs `FederationComposer` (Φ_tool∘Φ_agent∘Φ_model with residual checking) + `HyperplanePruner` (constraint half-space intersection). No new mechanism; this paper is the why-it-works foundation for what Research 380 already framed as shipped. PASS. Yun et al. [arXiv:2604.17148 "Graph-of-Agents (GoA)", ICLR 2026] — graph-structured LLM MAS via node sampling + bidirectional message passing + graph pooling. Decision structure (sparse relevant subset + bidirectional operator composition) ships as Sheaf-ADMM + Zone Density Routing + Set Attention. R368 lesson: LLM-call-as-compute-unit translates to constraint-operator-as-our-compute-unit. PASS.
