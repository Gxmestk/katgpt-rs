...
| 5 | App I associativity predicate: parallelizable ⇔ static Θ ∧ no **state** normalization ∧ linear bias-free final layer | YES (as tripwire/diagnostic) | kernel_opt corpus rules **queued: riir-clippy Issue 054 → Batch 87** (`state-normalization-breaks-associative-scan`, `dynamic-kernel-breaks-prefix-scan` + a B79 boundary rider); zero in-tree firing population (see §Associativity) — paper-distilled precedent = B79 itself |
...
None in this repo — no feature flag, no benchmark, no promote/demote (the R124 precedent). Executable remainder: **riir-clippy Issue 054 → kernel_opt Batch 87** (App-I boundary rules + B79 rider, plan opens at batch scheduling). Re-open triggers: (a) a served/adopted model with a TTT-KVB layer (→ run the 4 probes, then the App-I eligibility check), (b) any plan to pretrain a linear-attn/GDN layer (→ the reduction trajectory is the design table), (c) an in-tree kernel that normalizes recurrence *state* between updates (→ the Issue 054 rules fire immediately, promote to a hot batch).
