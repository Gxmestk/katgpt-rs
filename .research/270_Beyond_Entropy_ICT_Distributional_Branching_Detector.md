...
| G2 | Median inflection ∈ [5%, 20%] (paper §A.4.1 ~10%) | median 37.5% on synthetic-NPC suite | ⚠️ BORDERLINE-FAIL — paper's 10% is LLM-token-specific. Issue 033 (closed + removed; documented finding, k_percent tunable via API; recover via `git show fce6e44b^:.issues/033_ict_g2_inflection_37_percent_npc_domain.md`). Does NOT block G3. |
...
| Coherence-decay re-estimation | `riir-ai/crates/riir-engine/src/latent_functor/reestimation/mod.rs::tick` (Plan 303/305) | ✅ orthogonal signal — coherence decay ≠ JS-uniqueness |
