...
> **PASS-Redirects (synthesis):** "Towards Looped Models Done Right" [notion:ifm-research/Towards-Looped-Models-Done-Right, Huang/Shi/Chen/Wen/Liu/Xing/Ma, Aug 2026] — controlled ablation finding that iteration-specific expert selection matters causally for TRAINED looped MoE (Huginn MoE from scratch). This does NOT contradict our layer-mode (which pins routing for FROZEN MoE checkpoints to prevent thrash): layer-mode is correct when the model was NOT trained for looping; iteration-specific selection is a trained feature of looped-MoE pretraining. The MoE finding is → riir-train; our training-free layer-mode default stands. The sandwich-envelope finding (localized recurrence beats full-stack) is consistent with our depth-fraction rule (0.45–0.60 mid-stack window).
> **PASS-Redirects (synthesis):** Li, Li & Zhou [arXiv:2606.06574 "Skip a Layer or Loop It? Learning Program-of-Layers in LLMs"] (ICML 2026; extends their own search-only v1 arXiv:2507.07996) — Pass. Their MCTS structural findings (contiguous segments, K_max=4, ≤1 recurrence/segment, mid-stack skew) independently cross-validate our tf_loop window defaults (width-4 @ depth-fraction ~0.48, Plan 136) and are convergent evidence for the shipped HydraSkipPlan calibrated-skip substrate; the learned 2.1M program predictor is prior-art-covered by Dr.LLM [arXiv:2510.12773] and modelless-covered by our halters (GainCost/FPRM/ELT). Only uncovered piece: a JOINT skip+loop program search space (their F1: joint > loop > skip at equal budget) — no consumer today; revisit if a per-domain frozen-program use case lands.
> **PASS-Redirects (synthesis):** SMELT [arXiv:2609.01343 "SMELT: Scaling Laws for Compute-Matched MoE Looped Transformers"] (Wang & Zhang et al., Tsinghua/ByteDance Seed/M-A-P, Sep 2026) — trained looped-MoE pretraining scaling study (first to match FLOPs + params + KV simultaneously; middle-50% ×2; 1/r residual scaling; 6.8–18% training-FLOP savings at equal loss). Two relations to this note: (1) its 1/r looped-residual scaling is the TRAINED twin of our damped Euler 1/K sub-stepping — same form, opposite regimes (ours approximates the same t=1 endpoint for checkpoints never trained to be revisited; theirs is the stability rule for weights trained shared, where naive re-visits work because training placed them there — visit-2 amplifies aligned residual writes and cuts the attention sink); (2) its gains are a from-scratch pretraining effect (215B tokens) → riir-train, out of scope per the fusion ladder. Boundary strengthened: nothing in SMELT supports looping FROZEN non-looped checkpoints harder — our depth-fraction (0.45–0.60) + damping defaults stand; mid-layer re-execution on frozen Bonsai/qwen38 is not a quality lever.
