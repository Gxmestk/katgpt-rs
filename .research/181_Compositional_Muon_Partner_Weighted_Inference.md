...
> **PASS-Redirects (synthesis):** Xiao et al. [arXiv:2405.08553 "Improving Transformers with Dynamically Composable Multi-Head Attention" (DCMHA/DCFormer)] — PASS. DCMHA Theorems 2.1 + 2.2 prove attention-score composition ≡ QK projection composition (H-fold expansion) and attention-weight composition ≡ OV projection composition. The OV side maps directly to this note's "Hybrid OV rule" (V-side per-head msign, O-side global msign across heads); the QK side is the symmetric complement. DCMHA's contribution beyond what's covered here is the **dynamic, input-dependent composition map** generated by a trained FFN — that requires training from scratch (the architecture itself, → riir-train). The modelless distillates (static composition = Talking-Heads Attention; scale-normalized head-wise fusion) are already shipped in Plan 358 `ScaleNormalizedFusion` + Research 276 `PersonalityWeightedComposition`.
