...
**Verdict: MODERATE GAIN — Data structure optimizations transferable to our ToaST/BPE vocab lookup but Viterbi-specific algorithm is NOT used in our stack. Feature-gate the double-array trie as a vocab lookup acceleration path. Default-OFF until proven on our tokenizer workload.** *(Post-promotion update: proven on our tokenizer workload. **2026-08-16 correction — stale gate claim:** the earlier "`datrie_vocab` is now DEFAULT-ON in root Cargo.toml default array" does not match source; `datrie_vocab` is NOT in the root `default` array (`Cargo.toml` L180) — it is a forwarding alias to `katgpt-tokenizer/datrie_vocab` (L372, comment: "datrie is auto-routed" inside the toast_tokenizer path above `DATRIE_VOCAB_THRESHOLD`) and a member of `full` (L659). The §T3 conclusion "keep default-OFF, opt-in for large-vocab encode-heavy workloads" remains the operative state. Found during the Leis ART audit — see PASS-Redirects at end of file.)*
...
> **PASS-Redirects (synthesis):** Leis, Kemper, Neumann [ICDE 2013 "The Adaptive Radix Tree: ARTful Indexing for Main-Memory Database Systems", https://www.db.in.tum.de/~leis/papers/ART.pdf] — audited 2026-08-16: the mutable adaptive-node-size (Node4/16/48/256, path compression, lazy expansion) trie class ships NOWHERE in the 10-repo stack (this Datrie is the static build-once cousin; `Node4|Node16|Node48|Node256` = 0 grep hits across all repos), and every current consumer of its capability class is already served — static longest-prefix by this Datrie, mutable ordered prefix scan by riir-neuron-db `LocalKvStore::scan_prefix` (BTreeSet secondary index, O(log n + k), G2-gated). PASS — reopen triggers were filed as riir-neuron-db Issue 600, which is now **CLOSED, measured NEGATIVE (2026-08-18, riir-neuron-db Bench 485)**: the T1 ART-vs-dual-index bench ran on the real `LocalKvStore` at the pinned 50K-key Warm-tier scale and no ART arm satisfied T1's conjunction. `blart` as a drop-in `key_index` replacement uses **more** memory than the `BTreeSet` (1.053×); collapsing to a single ART reaches only 0.679× heap (not ≤0.5×) and regresses concurrent 8-thread point-`get` **3.4–4.5×** by surrendering papaya's lock-free read path; and a plain-`BTreeMap` control gets *better* memory (0.560×) plus ~90% of the scan gain, proving the win came from collapsing dual storage, not from radix. `congee` (the lock-free ART) is structurally ineligible — `K: Copy + From<usize>` caps keys at 8 bytes, Warm-tier keys are 12–16 variable-length bytes. **This Datrie remains the shipped trie substrate; the ART class is a measured dead end for this stack, not merely unadopted.**
