...
- **One DRY observation (CLOSED — false-DRY confirmed):** the encoder-decoder pattern (paper §3.6, App A.8) unifies `funcattn`, `cross_resolution_transport`, DEC integral operators, and DeepONet as instances of `inner-product encode → latent linear map → linear-combination decode`. Issue 042 investigated whether a `FunctionSpaceEncoderDecoder` trait should extract this. **T1 (2026-07-09) diffed the encode bodies in `funcattn.rs` Stage 2+3 (L878–898) vs `cross_resolution.rs::project_to_spectral_into` (L242–265): the encode math `v = Φᵀ · f` is identical but NOT extractable as shared code** — different batching `(n,d)→(k,d)` vs `(d,)→(k,)`, different memory layouts (contiguous vs strided columns → different SIMD primitives), different fusion (encode+normalize vs bare). A unified trait degrades to generic matmul (= the `TransportOperator` layer Plan 330 already ships). DEC `exterior_derivative` shares even less (topological incidence, no basis, no decode). **No impl; this is the forward-pointer so the DRY observation isn't re-raised.** Research 311 already explicitly dropped the encoder-half trait (`AnalyticLatticeEncoder`) as redundant with `FourierEncoder`.
