...
> **INTACT extension (Research 408, 2026-07-31):** [riir-train/.research/408_intact_shared_operator_dual_call_auxiliary.md](../../riir-train/.research/408_intact_shared_operator_dual_call_auxiliary.md) captures INTACT's (arXiv:2607.26056, [R462](462_INTACT_Isomorphic_Intent_To_Action_Search_Free_World_Models.md)) refinement of this `L_inv` line: the local branch *is* `L_inv` reformulated as displacement `m_local = z_{t+1} − z_t`; INTACT adds a second call `L_goal = −log p_η(a_t | z_t, sg(z_g) − z_t, a_{t-1})` through the **same** operator with stop-gradient on `z_g`. The sharing + asymmetric routing are the novel structural commitments; the gauge experiment (§5.2) is the causal proof. Consumer tracked in [riir-ai Issue 572](../riir-ai/.issues/572_intact_shared_operator_training_recipe_motor_gated_dec.md).
