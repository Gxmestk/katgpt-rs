...
**(b) `latent_functor/` operations.** The resolvent identity `M_l = R_l(I − γP̄^π_l)⁻¹` is *exactly* a latent functor: `(I − γP̄)⁻¹` is the functor application, `R_l` is the per-goal direction vector, and `M` is the composed operator. The Bellman inversion `P̂ = M⁺Q` is the **functor inverse** — given the composed operator and the output `Q`, recover the underlying kernel `P`. This generalises `crates/katgpt-percepta/src/wasm/interpreter/arithmetic.rs::extract_functor` from "extract displacement direction from source/target latent pairs" to "extract transition kernel from value-function snapshots". The coherence-decay re-estimation scheduler (`riir-ai/crates/riir-engine/src/latent_functor/reestimation/mod.rs`) is the natural place to trigger re-extraction when `coherence < τ_reest`.
