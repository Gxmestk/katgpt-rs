...
> **PASS-Redirects (synthesis):** Stokes, Brown, Flynn, Jamieson, Weinzierl [arXiv:2608.19273 "DSLHyPE—a DSL kernel language for the Exascale Hyperbolic PDE Engine ExaHyPE"] — the bilingual sibling of the kernel-DSL family (TK/Triton are monolingual; DSLHyPE keeps physics in native C/C++ and fuses it into the scheme's MLIR via Polygeist + enforced cross-language inlining). CubeCL remains our shipped one-source-multi-target mechanism; the paper's §VI missing-MLIR-optimizations list (kernel fusion, hidden AoS→SoA, scratchpad unification, step-DAG parallelism, tensor-contraction lowering) is covered by the riir-clippy kernel_opt corpus at higher resolution (B19/B26/B63 fusion, B46/B62 AoS→SoA + size gates, B28/B40 scratchpad, B70 DAG orchestration) — and their finding that canonical compiler passes miss these validates the hand-mined corpus approach → PASS.
