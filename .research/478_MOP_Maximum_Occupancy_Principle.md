...
1. **Open primitive** → `katgpt-rs` (this repo). The generic `MopSolver<N, A>` value-iteration operator (§2.1). Pure math, no game semantics. Feature flag `mop_path_entropy` (opt-in), benchmark in `.benchmarks/`, GOAT gate G1 (bit-identical to paper Eq. 7 reference Python implementation on the 4-room gridworld) + G2 (sub-ms at N≤256) + G4 (alloc-free via caller-provided scratch). **Status: planned, plan file to follow.**

2. **Architectural GUIDE** → `riir-ai/.research/NNN_per_npc_mop_runtime_guide.md` (next free number in riir-ai `.research`). Sells the runtime composition: `InducedCwmKernel` (frozen transition) + `MopSolver` (optimal policy) + HLA scalar projection (V* → "calm" direction) + risk-β from `SurpriseComputer` + CLR collective-threat composition. Documents what's public (the math) vs private (the runtime composition + game-IP transition kernels). **Status: planned — see "Next steps" below.**
...
2. ✅ **`.issues/NNN_mop_defend_wrong_poc.md`** in katgpt-rs — to be created next (tracks the §8 PoC obligation).
3. ⏳ **`katgpt-rs/.plans/NNN_mop_value_iteration_primitive.md`** — open primitive + GOAT gate. To be drafted; scope: `MopSolver<N,A>` value-iteration operator, paper-Eq-7 reference Python implementation for G1 bit-identity, sub-ms G2 target, alloc-free G4.
4. ⏳ **`riir-ai/.research/NNN_per_npc_mop_runtime_guide.md`** — private architectural guide for the CGSP×FORE×HLA×Induced-CWM fusion (§5 of this note expanded).
5. ⏳ **`riir-ai/.plans/NNN_per_npc_mop_runtime.md`** — runtime wiring plan, comes after the guide.

The plan + private guide files are scoped but not written in this session — they will be opened as follow-ups tracked by the issue. The note + issue satisfy the "mandatory outputs in this session" rule; the plans are execution artifacts that follow the guide per research skill §2 ("Super-GOAT plans come AFTER the guide — guide is strategy, plan is execution").
