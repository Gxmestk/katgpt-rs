...
> **PASS-Redirects (synthesis):** Tomašev, Franklin & Osindero [arXiv:2602.11865 "Intelligent AI Delegation"] — the DeepMind delegation framework's core runtime question ("when to delegate") ships here as the tri-gate's `Delegate` variant + `ceil_delegate` budget; its protocol-level half (escrow-bonded verifiable completion §4.8, trust/reputation §4.6, complexity floor §4.3) ships in riir-dapps/riir-chain (`MultiClaimEscrow` + `Settlement::None` default, `EvidenceTier`/`KarmaRing`, three-test rate gate) — external validation of the architecture, no new primitive, no actionable gap.
