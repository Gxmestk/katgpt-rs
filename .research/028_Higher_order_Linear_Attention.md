...
> **PASS-Redirects (synthesis):** Wang et al. "Zero Train-Inference Mismatch — now for linear attention, and under async RL" [blog https://yichuan-w.github.io/blog/GDN-train-inference-mismatch-asyncRL/ — Gated DeltaNet bitwise trainer/generator parity on TorchTitan RL + vLLM] — PASS 2026-08-07: training-infrastructure paper (TorchTitan unified model, batch-invariant GEMM, async off-policy RL pipeline); §3.5 Path 0 FAILS (no closed-form math to decompose — "batch-invariant" is a named kernel property, not a modelless primitive); §3.5 Path 0.5 NOT applicable (Plan 318 is SFT not async RL; no GDN model in flight). Our modelless runtime structurally avoids the train/inference mismatch — HLA inference already runs the intrinsically batch-invariant recurrent form ("one sequence's own state, fixed order, no cross-sequence reductions" is a property we have by construction, since we never run chunked — no backward pass), + freeze/thaw is bit-identical by BLAKE3 construction (Bench 019). Reverse-grep found HLA cross-platform determinism gap (Bench 001) but paper's batch-invariance axis ≠ platform-invariance axis. Conditional future Plan in riir-train if HLA training + async RL pipeline both come online.
