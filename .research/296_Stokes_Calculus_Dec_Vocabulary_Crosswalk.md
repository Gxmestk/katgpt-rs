...
> **Cross-ref (Gain):** Lee/Coeurdoux/Potaptchik/Du/Albergo/Vanden-Eijnden [arXiv:2608.01692 "Beckmann Transport Models: From Autonomous Flows to One-Step Maps"] — generative-modeling theory paper. The BTM divergence equation `∇·(νb)=μ0−μ1` IS the codifferential `δ` (this note's headline substrate). **Revised from PASS to Gain** (Research 468): the divergence equation is the modelless primitive our shipped CCE Moderator (Plan 295) explicitly documents as missing — the "MFG dynamics (occupation-measure flow constraints)" limitation that riir-ai Plan 325 shipped COMPLETE without closing. BTM's Beckmann OT framing provides the theoretical lens; the DEC `codifferential` IS the operator. The paper's training aspects (learning `b`/`T`) → riir-train; the EqM loss correction is image-FID only.
> **PASS-Redirects (synthesis):** Heightman, Orlova, Mantrov, Ustimenko [arXiv:2608.11911 "Hamilton-Zero: A Neural Tensor-Network Foundation Model for Ground States of Arbitrary Quadratic Qubit Hamiltonians"] — Hamilton-Zero's manifold calculus is the Lie-group sibling of our DEC substrate, a pure vocabulary crosswalk entry with no actionable wrapper: spin operators act as left-invariant vector fields (σ^a_i ↔ −iL^a_i; the Hamiltonian becomes a second-order differential operator on SU(2)^N, Eq 2.2) exactly parallel to our `exterior_derivative`/`codifferential` acting on cochains; and the Peter-Weyl physical-sector selection (project L²(SU(2)^N) onto the j=½ sector via per-site Casimir eigenvalue −Δ_iψ = ¾ψ; central oddness ψ(...,−q_i,...)=−ψ kills integer-j leakage by construction) is the representation-theoretic twin of `hodge_decompose`'s exact ⊕ harmonic ⊕ coexact projection onto the meaningful sector of a larger function space. Both are "constrain to the physical sector by construction, not by penalty". Their Casimir-penalty variant (Appendix A, restoring the variational bound for nonlinear-but-odd ansätze) is the sector-projection analog of our harmonic-projector regularizer — noted for vocabulary, no consumer → PASS.
> **PASS-Redirects (synthesis):** Bronstein, Bruna, Cohen, Veličković [arXiv:2104.13478 "Geometric Deep Learning: Grids, Groups, Graphs, Geodesics, and Gauges"] — the GDL blueprint survey; its Geodesics + Gauges chapters are the umbrella over this note's DEC substrate (exterior calculus IS discrete gauge-theoretic operators; `hodge_decompose`'s exact ⊕ harmonic ⊕ coexact = the gauge-invariant sector projection) plus CAT(0) geodesics (`cubical_nerve`) and Poincaré hyperbolic geodesics (`poincare_bridge`). Ships piecemeal across ~25 notes/plans (046 equivariant optimizers, 314 group invariance, 355 LieFlow symmetry discovery, 382 geodesic slerp, 446 GRAPE group representations, 457 SE(2) lift); the survey unifies but introduces no new mechanism → PASS.
> **PASS-Redirects (synthesis):** Stokes, Brown, Flynn, Jamieson, Weinzierl [arXiv:2608.19273 "DSLHyPE—a DSL kernel language for the Exascale Hyperbolic PDE Engine ExaHyPE"] — bilingual PDE-kernel DSL (PDE terms stay native C/C++ as black boxes; numerical scheme in a Python stencil DSL; both fused into ONE MLIR IR via Polygeist, optimization deferred to compiler passes). Our scheme/physics separation already ships as DEC-operators-vs-cochain-data (this note's substrate); the hyperbolic FV/FD machinery (Riemann solvers, Butcher tableaus, halo patches) has no consumer in our stack, and the compiler-level technique (cross-language inlining into MLIR) is not adoptable without building MLIR infra we deliberately don't own (CubeCL is the shipped IR-equivalent) → PASS.
