...
> **PASS-Redirects (synthesis):** MoR — Mixture-of-Recursions [arXiv:2507.10524 "Mixture-of-Recursions: Learning Dynamic Recursive Depths for Adaptive Token-Level Computation"] (Bae et al., KAIST/Google, Jul 2025) — applies Mixture-of-Depths-style routing (Raposo et al. 2024, the predecessor to dMoE's block-level routing) to recursive transformers: each recursion depth becomes an "expert" that selects top-k tokens via a trained router (expert-choice) or each token commits to a depth upfront (token-choice). The routing primitive (sigmoid score + top-k percentile + hierarchical filtering) is the same MoD pattern dMoE distills, but applied to recursion-depth selection rather than expert selection. §3.5 Path 0 fails: the router θ is a trained artifact co-trained with the recursive transformer from scratch — not a modelless construction. The architecture (weight-shared looping) is shipped as LT2 (Research 073 / Plan 108). Router training + MoR pretraining → **riir-train**.
> **PASS-Redirects (synthesis):** Minder et al. (EPFL/Toronto, West lab) [arXiv:2608.13482 "Synthetic Persona Pretraining: Alignment from Token Zero"] — PASS: SPP's Persona Selection Model framing (pretraining installs a repertoire of personas; post-training merely selects/binds one — it cannot install values the corpus never contained) is the training-side mirror of block-level routing: selection over a shared substrate scales where per-persona training cannot (their compute: one persona = 3B/500B-token pretrain + matched SFT + 13K GPU-hr reflection generation). No modelless primitive — the mechanism is a data+timing intervention in the training loop; the routing-side answer (block-level adapter routing over a shared base) is already this note's distillation.
> **GOAT Pillar:** ❌ Not a pillar — general inference optimization. Evaluated against [MMO GOAT Pillars](../../riir-ai/.docs/03_pillars/README.md): passes LoRA-independent (required) but fails MMO-product (required). Stays in `katgpt-rs` domain for modelless, `riir-ai` for LoRA routing.
