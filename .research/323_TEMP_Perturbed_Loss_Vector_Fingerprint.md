...
| **katgpt-rs/crates/katgpt-dec/src/** (DEC / Stokes) | `exterior_derivative d`, `codifferential δ`, `hodge_decompose` | The "fine-tuning direction" `v` is a vector field on the latent manifold; the Lipschitz bound is `|⟨d(loss), v⟩| ≤ bound` — a directional-derivative bound on the loss cochain | "Stokes-theoretic reframing: loss-vector spread bounds the directional exterior derivative of loss along `v`. Curse-of-dimensionality caveat: only meaningful for d ≤ 3 (HLA regions, KG embeddings) — NOT high-dim shards." |
...
| **DEC Stokes operators** (`katgpt-rs/crates/katgpt-dec/src/`) | `v` is a vector field on the latent manifold. Lipschitz bound `|⟨∇ΔL, v⟩| ≤ bound` is a directional-derivative bound on the loss cochain. `|⟨d(loss_cochain), v⟩|` is the directional exterior derivative. | Geometry-grounded interpretation; curse-of-dim caveat for high-d shards. |
