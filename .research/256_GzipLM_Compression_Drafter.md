...
**FINAL VERDICT (post-bench, 2026-06-17): GOAT FAILED on quest grammar gate (Plan 285, 2 runs).** nathan.rs/gzip-lm = beam-search text generation where each candidate is scored by `len(zlib.compress(corpus + ctx + candidate))`. Zero weights, zero training, the compressor IS the model. This IS a **fifth modelless primitive class** (corpus-as-model) alongside ConstraintPruners, Bandits, DDTree, SpeculativeDecode — that classification stands. But the original **Super-GOAT selling point (per-NPC plasma personality via SIMD LZ77)** was never validated, and the **GOAT fallback (Hot-tier quest grammar drafter replacing `TernaryDraftModel`)** also failed: G1 diversity 1.50× (need 3×), G2 latency 1077× (need ≤2×). Compression-based generation is fundamentally Warm-tier — beam search structurally needs ~1440 scorer calls/generation (~313µs) while template selection needs 1 matvec + 1 hash (~290ns). **What ships (opt-in, NOT promoted to default):** open primitive `compression_drafter` in katgpt-core (15/15 tests), private `CompressionQuestDrafter` wiring in riir-games (6+4 tests), `CorpusSnapshot` corpus-as-format with BLAKE3 commitment (the one insight that passed cleanly). `TernaryDraftModel` remains default-on for quest grammar. Two unexplored follow-up paths tracked in `.issues/029` (per-NPC corpus for G1, Warm-tier repositioning for G2) — neither pursued without concrete consumer. Honest negative result, fully documented.
