...
> **PASS-Redirects (synthesis):** Xiang et al. (RustForger / Rust-SWE-bench) [arXiv:2602.22764 "Evaluating and Improving Automated Repository-Level Rust Issue Resolution with LLM-based Agents"] — LLM-based code-agent framework (500-task Rust SWE benchmark + RustForger agent with proc-macro AST `Trace` command). PASS for the same reason as this note: the value is LLM-dependent semantic code generation (write reproduction test, propose corrective patch), not a modelless inference primitive. The `Trace` proc-macro is a tool serving the LLM agent's semantic analysis — our proc-macro substrate (`#[derive(ShardRow)]` in riir-neuron-db) is for shard encode/decode, different purpose. No modelless analog across all 7 repos. See R368 AutoMem for the contrast case where an LLM-agent paper's decision structure WAS implementation-agnostic and distilled modellessly; RustForger's core decisions (generate Rust source) are not.
