...
> **PASS-Redirects (synthesis):** Xiang et al. (RustForger / Rust-SWE-bench) [arXiv:2602.22764 "Evaluating and Improving Automated Repository-Level Rust Issue Resolution with LLM-based Agents"] — **split verdict.** The **RustForger agent** is PASS (LLM-dependent semantic code generation: write reproduction test, propose corrective patch — no modelless analog). The **Rust-SWE-bench DATASET** is **Gain** → captured in [Proposal 011](../.proposals/011_rust_swe_bench_latent_space_via_wasm_pruner.md) as a latent-space validation target: 500 (buggy→fixed) Rust source pairs from 34 popular repos, used as (a) a probe corpus for P010's `SourceFeatureAdapter` ridge-regression fit, (b) a functional test for P032's Phase 6 GOAT gate, (c) a target for the WASM-pruner in-loop validation pattern (moka-style: bring the benchmark INTO the inference loop via `WasmPruner` + rubrc). HIGHLY SPECULATIVE — same risk class as P010; the proposal exists to be evaluated and potentially rejected.
