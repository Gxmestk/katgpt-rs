...
**Status:** GAIN — theory lens + 2 actionable items filed (riir-train Issue 494: product-space LoRA merge + run-start feature-rank probe). NOT Super-GOAT (Q1/Q3 fail on mode-connectivity / model-merging published prior art). One verdict per track: modelless track Gain (certificate is open-glue over shipped erank substrate — consumer gap only), model-based track Gain (riir-train Issue 494). **LANDING ANNOTATIONS (riir-train, 2026-08-31, commits `4ede5770`/`15caadde`/`c8565cd7`/`cbfb175c`):** (1) Phase 1's prescribed product-space merge was DEMOTED by its own gate — the rank-r SVD re-factorization re-canonicalizes A's row space and the sigmoid does not survive it (probe parity 0.77–0.87 vs 0.999+); align-then-average landed instead. GAUGE NARROWING: for sigmoid-gated edges the ONLY exact function gauge is the PERMUTATION group — general orthogonal rotations mix gate arguments pre-nonlinearity (`σ(Qᵀx) ≠ Qᵀσ(x)`) and sign flips break σ; this §'s orthogonal-align framing narrows accordingly. (2) T2.2 real-data gate: alignment retained (as-trained cross-seed members share a near-gauge, aligned/naive 1.0010; gauge-twin corroboration on a real-trained seed: naive parity 0.0–0.945 vs aligned ≥0.99999). (3) T4's prescribed warning `feature_rank < N` is a MATHEMATICAL TAUTOLOGY — mean-centering puts the all-ones vector in the centered matrix's left null space, so erank ≤ N−1 IDENTICALLY (pinned: 8 exact basis directions → erank exactly 7.0); the landed warning is the paper's actual structural condition `hidden_width < N`, with the refined `target_rank > model_rank` comparison separating converged floors 133× in the T4.3 sweep. **CLOSE-OUT ANNOTATIONS (riir-train, 2026-08-31, commit `e153ef56` — Issue 494 fully landed + removed per the noise-reduction rule; its git history carries the phase-by-phase record):** (4) T2.3 cross-game merge gate: real-data verdict DIVERGENCE — the warning is PINNED. Same-game merged parity 0.9992 vs cross-game 0.5881 (gap 0.41 over the 0.05 bar); secondary finding: activation cosine is suggestive but NOT sufficient protection (4 collapsing positions at 0.00–0.32 parity had low/negative cosine; 4 high-cosine positions merged near-exactly) — production default recommendation = the new `Gate` policy (`consolidate_labeled`), `min_parity ≥ 0.95`. This confirms §Secondary's item 8 ("cross-game edge merging NOT licensed") with measured numbers. (5) T3 same-run checkpoint soup (`lora_soup`, opt-in): T3.2 GOAT measured NEGATIVE — soup LOSES held-out mean 0.33142 vs 0.31331 (+5.8%), seed variance +47%, arena win-rate 0.250; root cause measured: best==final at every seed (training still descending at epoch 8), so the last-4 window averages a good final point with worse mid-descent checkpoints — **same-run soup pays at CONVERGENCE, not mid-descent** (the paper's flat-basin interpolation premise requires an annealed/converged tail). Feature stays opt-in as the recorded negative artifact; the gate pins the losing assertions loudly, so a converged-recipe re-measure that flips them forces a deliberate re-pin. **RE-OPEN DISCHARGED + T2.1 CLOSED (riir-train, 2026-08-31, commits `3f5532ed`/`f6c1cf4c`):** (6) the armed converged-recipe re-measure ran and the verdict is NEGATIVE-HARDENED — soup loses held-out at 0/4 seeds at BOTH k=2 (×1.0197) and k=4 (×1.0102, bar ≤×1.01); k=4 shows the SWA variance-reduction signature (seed std ×0.83) but never recovers the mean; and the deeper structural finding: the convergence regime is UNREACHABLE by recipe changes — the `ceil(n2·j/k)` capture schedule sits at FIXED cosine phases (lr fractions {0.85, 0.5, 0.15, 0}), so the window always ends at the maximally-annealed point of the run and the constant-high-LR bouncing basin SWA needs is unrepresentable; `best == final` held at every probed rung (n2 ≤ 19200, lr ≤ 0.35, games ≤ 96, n1 ≤ 2000). Any soup revisit needs a capture-schedule change (constant/cyclical high-LR window over a flat basin), not a recipe change; the precondition gate fails loudly if the regime ever changes. Same-run soup is DEAD under this trainer. (7) T2.1 meter A/B (the "cosine suggestive but not sufficient" refinement): SVCCA (`katgpt_core::data_probe::svcca_into`, new light forward `edge_lora_cca`) TIES cosine's AUC (both 1.0000 — both order all 8 cross-game positions perfectly) but its separation margin is ~270× SMALLER (0.0017 vs 0.4589) with worse rank correlation (Spearman 0.76 vs 0.93) — root cause measured: subspace degeneracy, `var_keep=0.99` denoising retains only kx=1–2 dims on rank-4 edge outputs and the dominant canonical correlation exceeds 0.86 even across genuinely different objectives, so the meter has almost no dynamic range; SVCCA's load-bearing denoising step is what kills it here (the "noise" it truncates IS most of the representational difference on rank-4 edges). COSINE RETAINED — no production wiring change; the non-inferiority pin flips loudly if a future corpus (higher edge rank / wider probes, kx ≫ 2) ever favors CCA.
