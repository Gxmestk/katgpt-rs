...
> **PASS-Redirects (synthesis):** "World Model Anomaly Detection with a Latent Linear Prior" [TMLR openreview VLIzLK3CfR, code: anonymous.4open.science/r/lp-dreamer] — LP-DreamerV3 trains a linear matrix A with z_{t+1}≈A·z_t, flags imagined states where ‖z_{t+1}−A·z_t‖ exceeds threshold as delusional, and suppresses their contribution to value learning (+9% Atari100k at <90% value updates). PASS for our stack: the inference-time linear-prior anomaly detector already ships as KARC (R288) + `mcts_collapse_bridge` residual gate + `ReestimationScheduler` coherence<tau_reest trigger (this note §3.1); the value-update suppression in imagined rollouts is the MBRL-training half (DreamerV3 value backups from imagination) — a regime we do not operate in, → riir-train if MBRL value learning ever ships. No PoC needed: the quality claim is about a regime (Atari100k MBRL) we do not compete in — the AdaJEPA §3.6 defend-wrong precedent does not apply across-regime. Architectural coverage is at parity with the inference-time mechanism; the training-time gate has no current consumer to defend.
