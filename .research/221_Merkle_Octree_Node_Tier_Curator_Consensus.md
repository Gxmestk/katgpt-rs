...
> **PASS-Redirects (synthesis):** Leis, Kemper, Neumann [ICDE 2013 "The Adaptive Radix Tree: ARTful Indexing for Main-Memory Database Systems", https://www.db.in.tum.de/~leis/papers/ART.pdf] — audited 2026-08-16: "adaptive radix tree × Merkle commitment" is concept-saturated public prior art (Ethereum Merkle-Patricia Trie, Aptos/Diem Jellyfish Merkle Tree + maintained Rust `jmt` crate, LVMT/HMT research literature). No published system uses Leis-style adaptive nodes (Node4/16/48/256) inside a Merkle tree, but that internal node shape is invisible to the commitment scheme and would not survive a novelty claim. This MerkleOctree remains the shipped commitment-tree substrate; any future commitment-scoped prefix-tree work consumes `jmt` rather than hand-rolling. **Update 2026-08-18:** the reopen triggers (riir-neuron-db Issue 600) fired and FAILED — Bench 485 measured every ART arm against the real `LocalKvStore` and none beat it (`blart` key_index costs *more* memory than `BTreeSet`; a single-ART collapse regresses concurrent point-`get` 3.4–4.5×; `congee` cannot hold variable-length keys at all). Issue 600 is closed. The commitment-scoped zone-subtree delta-proof fusion idea is **unaffected** — it is a commitment/application question, and if built it consumes `jmt`, not an ART.
