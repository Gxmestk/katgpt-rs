...
> **Date:** 2026-08-14
> **Status:** Done (verification pass 2026-08-29 — resolutions re-verified in-tree, post-landing follow-ups appended to §4)
...
**Post-landing follow-ups (synced 2026-08-29, verification pass):**
- **Taught-rank persistence (riir-mmorpg-examples Issue 060, landed same day).** The learned coverage set now survives pet switch + re-login: `DemonstratedSkill::covered_mask`/`from_covered_mask` substrate accessors (verified in-tree 2026-08-29 — deliberately a SET, not a prefix, since consumers cover ranks out of order) → `PetEntry.taught_mask` → pet-roster **wire v2** (140→141 B, BLAKE3 commitment covers the mask) → Warm-tier WAL → STAR wire v4 → recovery rebuild → `on_pet_activated` restore. Design rule pinned en route: **derivable state stays off the wire, LEARNED state rides it** — `taught_mask` is learned, so it must cross; `element` (re-derivable from `monster_id`) did not. Commits: riir-ai `5a8508d6c` (substrate), riir-game-sdk `0d35c97`, consumer `ba9eb6f`.
- **Teaching-chain repair (riir-mmorpg-examples Issue 067, 2026-08-17, `bd04b73`).** After `shared_quest_monsters` promoted DEFAULT-ON, the live A/B was structurally impossible: hunting spawns moved authority-side under the claim path (a solo-harness hero can never swing → no demonstrations) AND nothing drained authority KILL batches into the teaching kill feed. Fix: A/B gates converted to `SharedWorldHarness` (authority + 1 peer = the production topology) + a cursor-guarded drain in `apply_kill_credit_system` republishing the peer's own credited kills as `(rank, pos)` — the exact payload the compiled-out local path used to feed. A/B restored with the Bench 013 differentiation (targeted closure 1.00 vs generic 0.67). Lesson for the fusion: the demonstration SOURCE is topology-dependent — under a shared world, demonstrations arrive as authority-credited kills, not local events.
- **Loop status:** the §2.3 consistency-gated demonstration-teaching loop is live end-to-end (substrate → SDK facade → consumer, with persistence), still opt-in (`demo_coverage_curiosity` DEFAULT-OFF — promotion is a gameplay decision per the CLR precedent, not a perf one).
