...
> **PASS-Redirects (synthesis):** Lee/Coeurdoux/Potaptchik/Du/Albergo/Vanden-Eijnden [arXiv:2608.01692 "Beckmann Transport Models: From Autonomous Flows to One-Step Maps"] — flow-matching variant using a time-independent drift `b(x)` (this crosswalk's §3 flow matching, but with the `t`-dependence dropped). The value is the theory: when μ1 is singular (lower-dim manifold M1), the autonomous flow `Ẋ_t=b(X_t)` still transports μ0→μ1 at first hitting of M1, with the drift satisfying the divergence equation `∇·(νb)=μ0−μ1` (= codifferential `δ`, crosswalk to Research 296). The one-step map `T` satisfies conservation `b·∇T=0`. Learning `b`/`T` requires gradient descent (FM regression loss Eq 5, Eulerian map loss Eq 15) → riir-train; the framework recovers Poisson Flow (ν=1) and corrects Equilibrium Matching's biased loss (image FID only). No modelless consumer in the 7-repo stack needs autonomous-flow transport. No new plan.
