...
|---|---|---|
| Delta encoder `z_t = g(x_{t-1}, x_t)` | `MicroRecurrentBeliefState::step(s_{t-1}, x_t)` (Plan 276, `crates/katgpt-micro-belief/src/attractor.rs`) — `state[i] = clamp(2·σ(W_s·s + W_x·x + b) − 1, ±clamp)`. A learned `(state, input) → next_state` kernel. **Structurally identical to DeltaTok's encoder.** | ✅ Encoder ships |
| Additive delta with clamp | `ReconstructionState::evolve_hla()` (`crates/katgpt-sense/src/reconstruction.rs:623`) — `hla[i] = (hla[i] + clamped_delta).clamp(-1, 1)` where `clamped_delta = clamp(lr·(normalized − half_total)·scale, max_delta)`. Family C leaky integrator. | ✅ Additive delta ships |
