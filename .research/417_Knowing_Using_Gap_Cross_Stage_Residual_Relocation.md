...
3. **R313 (Thinking-to-Recall, PASS)** — the latent-space analog of "knowledge stored but not used, recursively re-derive until it's reachable." `latent_functor/reestimation.rs` is the runtime mechanism that *triggers* when coherence decays; this paper is the diagnostic that *explains why* coherence decays (the latent state is stranded in the wrong stage).
...
- **R313 (Thinking-to-Recall, PASS)** — the latent-space analog. `latent_functor/reestimation.rs` handles the reactive case (re-derive when coherence decays); this paper's permeation map is the *diagnostic* that explains why, and the heuristic is the *proactive* complement.
