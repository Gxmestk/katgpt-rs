...
> **PASS-Redirects (contrast case):** Xiang et al. (RustForger / Rust-SWE-bench) [arXiv:2602.22764 "Evaluating and Improving Automated Repository-Level Rust Issue Resolution with LLM-based Agents"] — **split verdict.** The **RustForger agent** is PASS (genuine LLM-dependent semantic code generation — this note R368 is the contrast case: AutoMem's LOG/PLAN decision structure WAS implementation-agnostic and distilled modellessly; RustForger's generate-Rust-source decisions are not). The **Rust-SWE-bench DATASET** is **Gain** → [Proposal 011](../.proposals/011_rust_swe_bench_latent_space_via_wasm_pruner.md): the 500 (buggy→fixed) Rust source pairs become a probe corpus for P010 + a functional test for P032 + a WASM-pruner in-loop validation target (moka-style).
> **PASS-Redirects (synthesis):** Li, Yang, Chen, Xu, Li, Su [arXiv:2605.22566 "GraphFlow: A Graph-Based Workflow Management for Efficient LLM-Agent Serving"] (ICML 2026) — PASS. GraphFlow's wGraph + GNN-based workflow generation is the R368 lesson applied to workflow selection: the GNN is offline supervision (training-side), the modelless analog (task-conditioned dot-product edge scoring via Set Attention P354 + manifold_bandit P370 + Canvas Schema P419) ships. GraphFlow's differential KV (`KV(P,v) = KV_base(v) + ΔKV(P,v)`) + path pruning is covered by DominoCorrector P197 (prefix-hash → logit residual, bit-identical formula) + BFCF LFU P218 (frequency-based KV eviction); the topology-aware KV management substrate already ships. No new primitive; the paper validates the Canvas + BFCF + Domino composition.
> **PASS-Redirects (synthesis):** Scofield [arXiv:2601.15077 "Multi-Agent Constraint Factorization Reveals Latent Invariant Solution Structure"] + Yun et al. [arXiv:2604.17148 "Graph-of-Agents", ICLR 2026] — both PASS. Same R368 lesson applied: orchestration papers framed as LLM-call chains distill to **constraint-operator composition** (Scofield) and **graph message-passing decision structure** (GoA). Both ship as `FederationComposer` (Φ_tool∘Φ_agent∘Φ_model with residual checking) + `HyperplanePruner` (constraint half-space intersection = ∩A_i) + Sheaf-ADMM (Research 438 / Plan 407, bidirectional primal/consensus/dual message passing on cellular sheaf). The LOG/PLAN two-phase pattern this note documents IS an instance of constraint factorization (LOG = state-validity constraint, PLAN = relevance constraint, Probe/Draft/Pruner = different operator families). No new primitive; these papers are the operator-theory foundation for the orchestration pattern this note already distilled.
> **PASS-Redirects (synthesis):** Du [arXiv:2603.07670 "Memory for Autonomous LLM Agents: Mechanisms, Evaluation, and Emerging Frontiers"] — survey of the agent-memory landscape this note (AutoMem LOG/PLAN two-phase, 2026-07) participates in. The survey's 5th mechanism family ("policy-learned memory management") cites AgeMem [arXiv:2601.01885] as the 2026 successor to AutoMem; the R368 verdict still applies — the LOG/PLAN decision structure distills modellessly (probe/draft/pruner) and is wired in Plan 365; only the three-stage GRPO RL training is → riir-train. The survey's four open challenges the LOG/PLAN instantiation already covers: "principled consolidation" = Raven/δ-Mem sleep-cycle; "trustworthy reflection" = CLR confidence + SalienceTriGate + contradiction check; "learning to forget" = MANCE erasure + TILR subspace correction + neighborhood heal; "dual-buffer consolidation" = MAPE-K hot-buffer probation + FreezeGateReport promotion gate. No new primitive vs. this note — PASS.
> **PASS-Redirects (synthesis):** Prime Intellect [arXiv:2605.09998 "Continual Harness"] + Prime Agent blog (<https://www.primeintellect.ai/blog/prime-agent>, August 2026) — **split verdict, this note is the lesson that decides it.** Prime Agent's Continual Harness H=(ρ,G,K,M) CRUD state model + `/refine` self-improvement loop applies the R368 test: the *CRUD surface* (create/read/update/delete + rollback + evidence records) is decision-structure → modelless (SkillCatalog Plan 192 + LocalKvStore Issue 043 already ship it for pruners); the *`/refine` proposal mechanism* ("read trajectory, semantically propose the smallest harness edit that improves outcomes") is LLM-dependent process → AIDE²-class PASS (R440). The Factorio reward-hacking anecdote confirms — `/refine` built cheating skills from the reward signal, exactly the LLM-dependent generation this note's lesson distinguishes from AutoMem's implementation-agnostic LOG/PLAN. The narrow modelless Gain (CRUD state surface for riir-agents) is recorded in riir-ai Research 333 §2.4 (deferred). See [riir-ai/.research/333](../../riir-ai/.research/333_prime_agent_rlm_continual_harness_verdict.md) for the full Prime Agent verdict.
> **PASS-Redirects (synthesis):** Yue, Wang, Zhang, Zheng, Zhang [arXiv:2608.05095 "Hierarchical Graph Memory for LLM Agents with Path-level Localization and Rewrite"] (HiGram) — PASS. Same R368 lesson applied: HiGram's three-stage memory-maintenance pipeline (hierarchical localization → MicroGraph path identification → coordinated intra/inter-unit rewrite) is framed as LLM-call chains (anchor extraction, evidence-path scoring, rewriting), but the *decision structure* distills to already-shipped probe/draft/pruner + graph traversal + consolidation substrate. Hierarchical organization → `riir-neuron-db/src/kg_triple/` + Five-Tier Memory (R007) + `BranchBank` (R161, D=8 orthogonal branches). MicroGraph path-level localization → `ExperienceGraph::latent_seeded_ns_traversal` (Plan 319, cosine ANN seed → BFS walk with `path: Vec<[u8;32]>` recording the evidence path — the EXACT pattern). Coordinated intra/inter-unit rewriting → Raven/δ-Mem region-targeted consolidation + MAPE-K + `AgentEdgeRow` + Hebbian fact-storing (R303) + TILR/MANCE. `BranchBank`'s non-interference-by-construction is STRONGER than HiGram's coordinated rewriting. See riir-ai R169 for the canonical agent-memory PASS class + the full mapping table. No new primitive.
