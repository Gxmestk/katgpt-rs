...
**Routing:** No new primitive, no plan, no guide. Three `.issues/` entries created for the actionable gains (per §1.55 output discipline — PoC/proof tasks go to `.issues/`, not the note):
- `riir-ai/.issues/523` — measure optimal `ResidualField.layer` injection depth (gain #2)
- `riir-ai/.issues/524` — measure latent-vs-text compute savings ratio (gain #4)
- `riir-ai/.issues/525` — cross-class NPC communication: does activation-space similarity predict quality? (gain #5, unblocks R247 P3)

Gain #1 ("replace > sum > mean") is an honest negative result — does not transfer to our heterogeneous case, no issue warranted. Gain #3 (single-step sufficiency) is a pure design-principle validation of the existing per-tick `crowd_attention` design, no code change, no issue.
