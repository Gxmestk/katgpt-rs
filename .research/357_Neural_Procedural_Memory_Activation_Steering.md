...
> **PASS-Redirects (synthesis):** Zhang et al. [arXiv:2605.02735 "Visual Latents Know More Than They Say: Unsilencing Latent Reasoning in MLLMs"] — **PASS**. Their Stage I (query-guided contrastive latent–visual alignment with chunk-wise positive/negative patch sets) is the MLLM-visual instance of NPM's same mechanism: contrastive pair → steering vector injected into latent state. Our CNA (Plan 087) + Latent Field Steering (Plan 309) + CHaRS (Plan 409) cover this on 8-D HLA state (vs NPM's 4096-D LLM residual). Their query-relevance ranking is attention's standard dot-product (we have it); their chunk-wise disjoint positive/negative sets are the same anti-collapse diversity constraint as NPM's per-trajectory contrastive pair construction.
