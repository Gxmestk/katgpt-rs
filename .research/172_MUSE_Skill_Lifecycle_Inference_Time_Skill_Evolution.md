...
| Skill retrieval (which skill to use) | `BanditPruner<P>` with `BanditStrategy` | `crates/katgpt-ruliology/src/bandit.rs` | ✅ Production |
...
| Bandit feedback as skill selection | UCB1/Thompson/ε-greedy over pruner arms | `crates/katgpt-ruliology/src/bandit.rs` | ✅ Production |
...
> **PASS-Redirects (synthesis):** Ning et al. [arXiv:2608.05446 "EvoHarness-RL: Learning Self-Evolving Runtime Harness for Long-Horizon LLM Agents"] — skill-store EVOLUTION confirmed as the load-bearing half: its epoch-boundary add/update/REMOVE/skip consolidation + LFU eviction + batch-stable store are the training-time mirror of this note's ITSE lifecycle (coverage unchanged); the contradiction-triggered REMOVE operator is recorded as a consumer-less micro-delta in riir-train Research 436 (re-open trigger: any revisable-prior store). Training-recipe deltas: riir-train Plan 348 Item G.
