...
> **PASS-Redirects (synthesis):** Ueyama [arXiv:2608.23228 "mold: A Massively Parallel Linker"] — same class as §5: hugely valuable for its own audience (linker/build engineers), while we consume its PRODUCTS not its techniques (papaya descends from mold's concurrent hashmap; parallelism discipline already codified in rust-optimize; mold is ELF-only so inapplicable on our macOS/Windows boxes). The decouple-entangled-phases-via-fixpoint pattern already ships (riir-clippy bounded-fixpoint edit application; Bevy `.after()` pins). PASS.
