...
| **C** | **Block-dimensionality diagnostic** | Stable rank of a block = effective number of dimensions it uses. Already shipped as `effective_rank` / `stable_rank_update_into` (Roy-Vetterli standard `‖O‖_F² / ‖O‖_op²`). | `crates/katgpt-core/src/data_probe/geometry.rs::effective_rank`, `crates/katgpt-core/src/data_probe/sink_classify.rs::stable_rank_update_into` (Plan 287). **Identical metric, already shipped.** `participation_ratio` (Plan 301) — continuous analog. |
...
- **Stable rank (shipped)**: `katgpt-rs/crates/katgpt-core/src/data_probe/sink_classify.rs::stable_rank_update_into` + `crates/katgpt-core/src/data_probe/geometry.rs::effective_rank` (Plan 287).
