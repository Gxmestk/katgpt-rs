...
>
> **PASS-Redirects (synthesis):** Liu & Han [arXiv:2606.11722 "ICA Lens: Interpreting Language Models Without Training Another Dictionary"] — Gain (NOT PASS, goes to Plan 475). ICA Lens is the **no-verdict-label cousin** of MAG: where MAG mines contrast directions from the model's own verdict `y_M`, ICA mines directions of maximum non-Gaussianity from the unconditional activation distribution — no verdict signal needed. The two are complementary (high overlap = verdict captures natural structure; low overlap = structure the verdict misses). The FastICA fixed-point iteration (non-Gaussianity-maximizing rotation after whitening) is genuinely missing — our `EigenbasisTracker` does PCA (variance-maximizing), which is strictly weaker on non-Gaussian data. Ships as `ica_lens` feature in `katgpt-spectral` (Plan 475). Plus the ERF (Effective Receptive Field) diagnostic is novel — measures per-direction context dependence, the cognitive-hierarchy signal the latent_functor tier-routing needs.
>
...
| External escalation | AnyRAG gateway (riir-neuron-db/src/gateway.rs) | **MAG transfer ranking = which source to escalate to** |
