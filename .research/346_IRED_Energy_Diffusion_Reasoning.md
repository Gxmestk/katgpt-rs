...
| Gibbs / energy-weighted retrieval | `opus::boltzmann_probabilities` (τ-controlled); PlackettLuce Gibbs sampling; **Research 317's `1/E²` sharpening** | `crates/katgpt-pruners/src/opus/boltzmann.rs`; R317 |
...
| Trajectory energy = length-normalized NLL | `spectral_entropy_dct`; `ac_prefix::conditional_logprob` (computes exactly this) | `crates/katgpt-attn/src/chiaroscuro/entropy.rs`; `crates/katgpt-core/src/ac_prefix/` |
