...
> **PASS-Redirects (synthesis):** Yin & Wang [arXiv:2501.15857 "Are Transformers Able to Reason by Connecting Separated Knowledge in Training Data? (FTCT: Fragmented at Training, Chained at Testing)"] — PASS for the same reason the IMIR training-theory half (Theorems 2 & 3, gradient confinement, lottery ticket) is PASS: training-dynamics theory → riir-train. FTCT's mechanistic-interpretability finding (training discovers induction heads + parent-retrieving attention assignment) is a sibling observation to IMIR's ICL/IWL circuit-competition analysis, but with weaker modelless residue — FTCT does not derive a commutant-style basis or a greedy-ablation algorithm; its only inference-time residue is the 2-layer Transformer existence construction (Lemmas 5.1–5.3), which is a theoretical proof, not a deployable primitive. The λ=M/N ≥ 0.3 phase-transition rule for compositional generalization emergence is a training-data design insight → riir-train.
> **PASS-Redirects (synthesis):** Fu et al. [arXiv:2608.03913 "Sparse Weight Decomposition for Efficient Circuit Extraction"] — PASS. SWD does the SAME job as IMIR §5 (auto-circuit-detection via greedy backward elimination on factored coordinates) but with a WEAKER basis: SWD's circuit units are the shared intermediate coordinates of a *learned* sparse two-factor decomposition (S₁·S₂), whereas IMIR's circuit units are coordinates of a *closed-form* commutant basis derived from the data-symmetry group. IMIR's symmetry-defined basis is the stronger primitive (no fitting needed, group-theoretically grounded); SWD's sparse factors require <1% data to fit but are not symmetry-derived. SWD's mech-interp value (circuit extraction) is not our commercial domain; the sparse factorization mechanism ships as R231 (`SparseTaskVector`); concept ablation on our runtime substrate ships as R409 (MANCE). No plan created.
...
| ICL/IWL split × freeze/thaw | Theorem 2's "IWL starves ICL" as a runtime pattern: frozen committed state dominates until curiosity explicitly overrides. | Already ships as `riir-ai/crates/riir-engine/src/latent_functor/reestimation/mod.rs` coherence-trigger. No new code. |
...
| **riir-ai** (private runtime) | NO for the residue | The per-NPC ICL/IWL analog already ships as `riir-ai/crates/riir-engine/src/latent_functor/reestimation/mod.rs`. No new runtime IP. |
