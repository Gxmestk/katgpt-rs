...
> **PASS-Redirects (synthesis):** Schusterbauer et al. (CompVis) [arXiv:2312.07360 "Boosting Latent Diffusion with Flow Matching"](https://arxiv.org/abs/2312.07360) — PASS. Cascaded image-synthesis architecture (ECCV 2024 oral): a small frozen diffusion model supplies semantic diversity, a trained flow-matching stage transports the coarse latent into a high-dimensional latent space (resolution/detail boost), and a frozen convolutional decoder renders 1024²–2048² at reduced compute. The contribution is ARCHITECTURAL (where to spend the trained capacity: semantics small + detail cheap), not mathematical — the FM training is standard rectified-flow velocity regression with no closed-form modelless content to extract. We ship no generative image models and train no image diffusion; the cascade insight (small semantic core + cheap refinement stage) is already instantiated in the stack as drafter/verifier speculative decode, swarm leader/follower LOD, and the KARC/ReestimationScheduler/LT2 coarse-to-fine refinement cited above. Same verdict class as the 2608.05811 redirect.
