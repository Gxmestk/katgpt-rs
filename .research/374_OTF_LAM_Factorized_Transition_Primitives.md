...
> **PASS-Redirects (synthesis):** Wei et al. [arXiv:2607.09185 "Causally Debiased Latent Action Model for Embodied Action Conditioned World Models"] — CD-LAM addresses the SAME latent-action-confounding problem this note solves, but via a TRAINING-TIME debiasing route (three fine-tuning losses: embodiment-centric reconstruction w/ SAM3 foreground masks, action-centric contrastive softplus, free-bit KL + zero-transition calibration) rather than the modelless factorized-codebook route shipped here. §3.5 Path 0 fails for CD-LAM: L_emb/L_ctr/L_cal are genuinely gradient-descent losses that don't decompose into modelless primitives; paths 1–3 (freeze/thaw, deterministic LoRA, latent projection) cannot remove bias baked into trained weights without retraining. CD-LAM → riir-train. The modelless solution to the same problem (factorized codebook + sigmoid relevance gate suppressing distractor primitives) already ships as `factorized_action` (Plan 375, GOAT PASS). CD-LAM's contrastive term has a separate modelless cousin: CNA (Research 053 / Plan 087, `cna_steering`).
