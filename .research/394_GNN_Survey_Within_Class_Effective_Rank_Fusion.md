...
> **PASS-Redirects (synthesis):** Liu & Han [arXiv:2606.11722 "ICA Lens: Interpreting Language Models Without Training Another Dictionary"] — Gain (NOT PASS, goes to Plan 475). ICA Lens's whitening step IS the eigenvalue-spectrum decomposition this note's `effective_rank` operates on — the paper's Figure 3 (ICA directions more non-Gaussian than PCA directions) is the per-direction analog of the within-class-vs-global rank contrast. The FastICA fixed-point iteration (non-Gaussianity-maximizing rotation after whitening) is the missing joint optimization that this note's post-hoc `effective_rank` + `excess_kurtosis` ranking approximates only heuristically. Ships as `ica_lens` feature in `katgpt-spectral` (Plan 475).
>
