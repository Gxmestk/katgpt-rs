...
> **PASS-Redirects (synthesis):** Chen, Lin, Wang [arXiv:2607.28457 "SVR: Self-Verifying Refinement via Joint Verdict–Confidence Reinforcement Learning for Adaptive Test-Time Compute"] — PASS on the modelless side, same pattern as this note (R363). SVR is multi-turn GRPO training where the policy learns to produce verdict+confidence for adaptive compute allocation; the runtime analog (state-dependent compute budget via self-generated verdict+confidence) already ships modellessly as `EarlyStopGate<P, confidence_threshold>` + `SalienceDecision` + `CompactionDecision` + `CommittedFieldBlend::commit`. SVR's training contribution (Joint Verdict–Confidence RL with Brier calibration reward) requires ground-truth `y` — not modelless-decomposable per §3.5 Path 0. Per Path 0.5 → Plan in riir-train/.plans/328. R363 and SVR are the same paper class: training-only contribution + runtime analog already shipped under different vocabulary. The only delta vs R363 is that SVR's signal is internal (policy-generated verdict+confidence) while R363's is state-dependent budget `k`; both reduce to "the runtime commit/refine/forced gate already ships — the training is what produces the calibrated signal".
...
|---|---|---|---|
| **Asynchronous Plasma-tier NPC cognition** | Bridge the 20Hz tick budget for hard NPC decisions by running deep cognition on a warm-tier worker while the plasma-tier reflex policy holds the NPC's actions for `k` ticks | Mirror the paper's committed-action protocol: reflex policy = HLA evolve_hla single-step (already sub-µs); planner = full `latent_functor` application + CLR vote; on tick `t+k`, apply planner output. The `k` is selected per-NPC by `GainCostLoopHalter` (P304). | **Issue 364** (perf engineering, not research) — riir-ai/.issues/364 (removed per noise-reduction rule) |
| **Variable-duration CGSP** | Generalize CGSP from fixed-`k` rollouts to per-NPC-`k` rollouts where `k` comes from gain/cost halting, with `γ^k` discount correction (Appendix C) carried through GAE | **Honesty correction (2026-07-02):** the original framing ("replace γ with γ^k in the advantage computation") overstated the scope — CGSP's update path is bandit/priority-based (`PriorityTableBandit` + `hint_delta`), NOT a γ-discounted advantage. There is no γ to replace. The actual ship is a **staleness weight** `exp(-λ·(k_npc−1))` on the priority hint_delta (the bandit-update analog of γ^k); the literal γ^k correction is a deferred hook for any future value/advantage path. **Phase A COMPLETE (2026-07-02):** `staleness_weight` + `CgspConfig { k_npc, staleness_lambda }` shipped in `katgpt-rs/crates/katgpt-core/src/cgsp/loop_.rs`, applied at the `HintDeltaBandit::absorb` site in `CgspLoop::cycle()` Step 7. Default config bit-identical to pre-issue behavior (verified by `assert_eq!` with k_npc=1 vs k_npc=99). G1/G2/G3 PASS. See Issue 365 §"The correction". | **Issue 365** (Phase A complete) — riir-ai/.issues/365 (removed per noise-reduction rule) |
...
| Repo | What | Why |
|---|---|---|
| `katgpt-rs` | `staleness_weight` + `CgspConfig { k_npc, staleness_lambda }` (Issue 365 Phase A, 2026-07-02) | Bandit-update analog of γ^k; applied at `HintDeltaBandit::absorb` in `CgspLoop::cycle()`; default config bit-identical no-op |
...
- **Q: Is the two-GPU asynchronous protocol worth adopting for riir-ai's hot/plasma tier?** Possibly — the paper's "reflex holds the agent while planner computes asynchronously" pattern maps cleanly to "HLA evolve_hla holds the NPC while latent_functor computes on a warm worker". Tracked as **Issue 364**.
- **Q: Does the `γ^k` GAE correction improve CGSP value estimates?** **Honesty correction:** CGSP has no γ-discounted value estimates — it's bandit/priority-based. The modelless analog (staleness weight on the priority delta) shipped as **Issue 365 Phase A** (2026-07-02): `staleness_weight(k_npc, lambda) = exp(-λ·(k_npc−1))` applied at the `absorb` site, default-off (bit-identical no-op), G1/G2/G3 PASS. The literal γ^k correction remains a deferred hook for any future value/advantage path.
