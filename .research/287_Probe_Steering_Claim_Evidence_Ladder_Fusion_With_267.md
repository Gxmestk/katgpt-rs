...
| HLA `evolve_hla` (`crates/katgpt-sense/src/reconstruction.rs`) | L1 (state update with no current downstream-causal claim). Research 286's magnitude-drift finding is itself an L1 finding *about* this kernel — it observes a behavioral regularity (drift) without yet proving the post-norm fix causally removes it. |
...
|-----------|-----------|---------------|------------------------|-------------------|
| `EmotionDirections::project` | `crates/katgpt-pruners/src/emotion_vector.rs` (Plan 162) | Detection | **L1** (read of current emotion projection) | L2: show downstream action-selection change across ≥3 paraphrases + benign-shift control. L3: predict-control parity for desperation direction. |
...
| `FutureBehaviorProbe` (FPCG) | `crates/katgpt-pruners/src/future_probe.rs` (Plan 292, in progress) | Prediction | **L1** (planned; blocked on offline training, Issue 032) | L2: Benchmark 292 Pareto frontier across ≥3 paraphrases + seeds. L3: predict-control parity + specificity (out of current scope). |
| `PosteriorGuidedPruner` | `crates/katgpt-pruners/src/posterior/wrapper.rs` (Plan 239) | Detection (Bayesian precision) | **L1–L2** (records evidence + gates; gain measured) | L2: generalization across regime shifts. L3: n/a (precision tracking is not a causal claim). |
| HLA `evolve_hla` | `crates/katgpt-sense/src/reconstruction.rs:623` | Detection (latent state update) | **L1** (state update; no downstream-causal claim) | L1+: Research 286 magnitude-drift finding. L2: show HLA delta influences action selection across freshness window. |
...
> **Cross-ref (2026-08-27; updated 2026-08-28 with the measured instance):** Hanna & Ameisen [arXiv:2604.12493 "Latent Planning Emerges with Scale"] — the published L3 exemplar this rubric predicts: intervention + specificity control (random-feature arms, App. F) + direct-vs-full-effect split, all on planning claims; and a measured C9 instance (App. G: probes 0.6–0.7 F1, patching <10% output change — probe language downgraded to L1 in their framework, exactly the vocabulary enforcement this note mandates). The two-condition planning test (forward/backward licensing) **is now a landed concrete L3 instance** (riir-ai Issue 762 T1–T6 / Research 348 §13, `riir-poc::causal_planning_poc`): the authored-planner arm's BuySalve goal measured PLANNED — zero-ablation kills consumption (P 1.0→0.0) AND amplification advances its preparatory milestone (+7.19 ticks) beyond both substitution floors with an exactly-inert specificity control — which licenses the L3 verb pair for authored planning claims ("causally controls"/"mechanistically mediates") on the hero cognition stack. The emergent (motivation-only) arm measured FORWARD-ONLY — emergent-planning claims stay at L1/L2 vocabulary until a passing instance exists (Super-GOAT Q2 stays NO).
