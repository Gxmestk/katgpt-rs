...
| Sample-M-candidates → score → argmax-select | CGSP Conjecturer+Guide loop (`crates/katgpt-core/src/cgsp/loop_.rs`); CompressionDrafter beam (`riir-ai/crates/riir-games-quest/src/quest_grammar/compression_draft.rs`) | Mechanism skeleton | ✅ shipped (different domains) |
...
> **Cross-ref (2026-08-27):** Hanna & Ameisen [arXiv:2604.12493 "Latent Planning Emerges with Scale"] — their App. G is direct *published* evidence for this note's detect-vs-predict gap on planning claims specifically: animal probes hit F1 0.6–0.7 yet full activation patching at the probe-localized position changed the output animal in <10% of cases — probe-found features were detection-side, not prediction-side. Distilled as riir-ai Research 348.
