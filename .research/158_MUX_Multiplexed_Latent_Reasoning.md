...
>
> **Citation-hygiene note (2026-08-25, Coconut v4 panel):** the RiM figures in §2.4 below ("+12-18pp improvement over direct-answer SFT", "7× faster than Coconut") are **uncited prose** — no benchmark in either repo backs them (Plan 172's own gates were cost-only: TTFT <1%, throughput <2%; no accuracy arm). Treat as unverified until a G-COCO-style arm produces a citation (riir-train Plan 352 row 4). Coconut itself (Hao et al., [arXiv:2412.06769 "Training Large Language Models to Reason in a Continuous Latent Space"]) re-examined same day: **PASS** — v4 cosmetic over v3; this note's superposition-BFS (§1.3) already ships as `mux_bfs`; only §4.4's height-confidence law escaped → katgpt-rs Issue 688.
---
