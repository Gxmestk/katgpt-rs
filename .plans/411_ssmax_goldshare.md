...
**Target:** `katgpt-rs/crates/katgpt-core/src/parallax_attn.rs` (SSMax extension) + `katgpt-rs/crates/katgpt-core/crates/katgpt-core/src/data_probe/gold_share.rs` (new diagnostic) + Cargo features `ssmax_temperature`, `gold_share_probe`
...
- [x] **T1.4** Implemented `crates/katgpt-core/crates/katgpt-core/src/ssmax.rs` types: `SsmaxMode` enum (`Fixed { s_l }` / `Adaptive { rolling_delta }`), `SsmaxConfig` bundle, default `Fixed { s_l: 1.0 }`.
- [x] **T1.5** Implemented `apply_ssmax_inplace(logits, mode, log_n)` — chunked 8-wide SIMD-friendly in-place multiply.
- [x] **T1.6** Implemented `crates/katgpt-core/crates/katgpt-core/src/data_probe/gold_share.rs` (pre-existing skeleton from prior session, kept as-is): `GoldShareReport` with the 5 plan fields (`gold_norm`, `total_norm`, `gold_share`, `gold_pre_softmax_max`, `noise_gap`), `gold_share`/`gold_share_flat` with `GoldShareScratch` (caller-owned, zero-alloc hot path).
...
- [x] **T1.8** Wrote 13 unit tests in `crates/katgpt-core/crates/katgpt-core/src/ssmax.rs` (all PASS): fixed/adaptive mode resolution, clamping (tiny/huge/zero/negative delta), multiplier math, in-place scaling bit-exactness, SIMD+remainder paths agree with naive, identity-at-multiplier-one, empty-slice no-op, `SsmaxConfig` caching.
- [x] **T1.9** Wrote 8 unit tests in `crates/katgpt-core/crates/katgpt-core/src/data_probe/gold_share.rs` (all PASS): all-true/all-false masks, empty-gold-set, degenerate all-zero, paper's Table 1 toy (4-head 8-key half-gold), pre-softmax-max + noise-gap correctness, flat/typed agreement, scratch ensure_capacity no-op.
...
- [x] **T2.4** Added `tiled_attention_forward_ssmax` to `crates/katgpt-core/crates/katgpt-core/src/attention.rs` behind `#[cfg(all(feature = "tiled_attention", feature = "ssmax_temperature"))]`. Folds `s_L · log(N)` into the softmax scale — the zero-overhead way to apply SSMax to flash-attention (no score matrix materialization).
...
- [x] **S1** A `belief_share` analog of GoldShare for per-NPC latent state — does the NPC's projection still carry its personal signal, or has it been drowned by aggregate crowd projections? **RESOLVED 2026-07-07 (DEFENDED)** — filed as `riir-ai/.issues/373_belief_share_npc_latent_dilution.md`, PoC ran (`riir-poc/benches/belief_share_npc_dilution_poc.rs`), verdict: personal signal survives at `belief_share ≥ 0.98` across all configs including the faction-dominated worst case. The COMMUNITY layer's faction aggregation contributes at most ~1.7% of the composed output norm (8 personal layers outweigh 1 faction layer). `belief_share` is a pure diagnostic; no behavior change or GOAT gate needed. PoC Addendum recorded in Research 392 §2.4. Path correction: the per-NPC emotion latent state lives in `riir-ai/crates/riir-engine/src/cgsp_runtime/types.rs` (`NpcEmotionScalars`), NOT `hla/` (which is the transformer Higher-order Linear Attention cache).
