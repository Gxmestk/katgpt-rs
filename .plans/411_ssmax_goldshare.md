...
- [x] **T2.1** Extended `ParallaxConfig` in `crates/katgpt-core/src/parallax_attn/mod.rs` with `#[cfg(feature = "ssmax_temperature")] pub ssmax: Option<crate::ssmax::SsmaxMode>` (default `None`). Manual `Default` impl updated with the cfg-gated field.
...
- [x] **S1** A `belief_share` analog of GoldShare for per-NPC latent state — does the NPC's projection still carry its personal signal, or has it been drowned by aggregate crowd projections? **RESOLVED 2026-07-07 (DEFENDED)** — filed as `riir-ai/.issues/373_belief_share_npc_latent_dilution.md`, PoC ran (`riir-ai/crates/riir-poc/benches/belief_share_npc_dilution_poc.rs`), verdict: personal signal survives at `belief_share ≥ 0.98` across all configs including the faction-dominated worst case. The COMMUNITY layer's faction aggregation contributes at most ~1.7% of the composed output norm (8 personal layers outweigh 1 faction layer). `belief_share` is a pure diagnostic; no behavior change or GOAT gate needed. PoC Addendum recorded in Research 392 §2.4. Path correction: the per-NPC emotion latent state lives in `riir-ai/crates/riir-engine/src/cgsp_runtime/types.rs` (`NpcEmotionScalars`), NOT `hla/` (which is the transformer Higher-order Linear Attention cache).
