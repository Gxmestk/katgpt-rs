...
- [x] T1.1: Add `CrossScaleConfig` struct to `src/speculative/dd_tree.rs`
  - Fields: `enable: bool`, `scale_alpha: f32` (default 0.5), `consistency_threshold: f32` (default 0.1)
  - `#[repr(C)]` for predictable layout
- [x] T1.2: Implement `branch_velocity_at(depth, marginal_slice) -> f32` — discrete velocity at depth
  - Computes: `marginal[depth] - marginal[depth-1]` (change in top-1 probability)
  - Zero-alloc: operate on existing marginal slices
- [x] T1.3: Implement `cross_scale_consistent(v1, v2, alpha) -> bool` — check consistency
  - Returns `|v2 - alpha * v1| < threshold`
  - Inline, branch-free: `f32::abs(v2 - alpha * v1) <= threshold`
- [x] T1.4: Integrate into `build_dd_tree_screened()` — filter branches that violate consistency
...
- [x] T1.5: GOAT proof: `tests/test_recfm_ddtree.rs`
...
- [x] T2.4: GOAT proof: `tests/test_recfm_lt2.rs`
...
- [x] T3.3: Modify `BanditSpeculator::speculate()` to check cross-hop velocity
...
- [x] T3.4: GOAT proof: `tests/test_recfm_spechop.rs`
...
- [x] T4.1: Add `recfm` feature flag to `Cargo.toml` (optional, depends on nothing new)
- [x] T4.2: Add `recfm` to feature list in README under "Gated Features"
- [x] T4.3: Ensure all three components are behind the same feature gate
- [x] T4.4: Run full test suite with `--features recfm` — zero regressions
