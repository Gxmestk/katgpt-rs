...
## Phase 5 — Private follow-ups (implemented across riir-* repos)

- [x] **P5.1 (riir-ai)** CLR + renoise-CE fusion arm (F1) — add renoise-CE as a third vote arm in CLR's sharpening gate. **DISCHARGED by riir-ai Plan 395 (CLOSED 2026-07-07, all 4 phases `[x]`).** `clr_renoise_fusion` feature is **DEFAULT-ON** in `riir-games/Cargo.toml` (commit `71612f62`, Phase 3 promotion). Implementation: `riir-games/src/npc_clr/renoise_fusion.rs` ships `RenoiseFusionConfig` (perturbation_level, k_draws, beta, lambda), `fused_reliability` combiner, `perturb_reverify_drift` (perturbs claim embeddings → re-verifies through frozen verifier → verdict drift), `NpcClrCycle::tick_fused` entry point, `BETA_ZERO` = bit-identical to CLR-alone (G3 PASS: 0 mismatches). **GOAT gate G1–G6 results:** G1 PASS (88% vs 0% on adversarial reliability ties, +88pp); G2a informational (3.73× ratio); G2b PASS (3.8µs/tick, 13.1× headroom); G3 PASS (0 mismatches at β=0); G4 PASS (default-off clean); G5 PASS (0 incremental allocs); **G6 MARGINAL** (+0.00pp on realistic 3-component margin-mixture traces, 3.35% disagreement rate, both winners hold up 100% — CLR's `(mean)^M` gate pre-filters brittle trajectories so the fusion has nothing to correct; T4.3 decision = KEEP default-on as harmless/cheap with β=0 floor). See `riir-ai/.benchmarks/395_g6_real_trace.md` for the honest root-cause analysis. Plan 395 original module shipped as WIP from a parallel agent — no duplication occurred when P5.1 was picked up.
- [x] **P5.2 (riir-ai)** Proactive functor stability probe (F2) — perturb observation buffer, re-estimate direction into scratch, measure cosine drift. Extends `ReestimationScheduler` (Plan 303). **DONE — implemented as `renoise_functor_probe` feature** (commit `7e16719f` on riir-ai `develop`): `RenoiseCeTrigger` config (perturbation_level, drift_threshold, k_draws), `with_renoise_ce()` builder, `collect_renoise_brittle_into()` proactive pass in `tick()`, `renoise_triggers` field on `TickReport`, `cosine_distance` helper. Follows the `JsUniquenessTrigger` pattern exactly. 7 tests pass (brittle-flagged, stable-not-flagged, zero-perturbation, determinism, re-estimation, cosine sanity, default-off). Clippy clean. Feature-isolated (default-off, bit-identical when disabled).
- [x] **P5.3 (riir-neuron-db)** Proactive freeze-gate probe (F5) — perturb wake-event embeddings, re-run `sleep()`, measure `weight_delta` drift. Extends `can_freeze` (Plan 002). **DONE — implemented as `renoise_freeze_probe` feature** (commit `725eed1` on riir-neuron-db `develop`): `proactive_freeze_drift()` method (k-draw perturb-and-re-average → MSE drift vs clean delta), `can_freeze_proactive()` companion (downgrades brittle shards), DRY `average_embeddings()` extraction (shared by `sleep()` + probe). Uniform perturbation model, MSE drift metric. 6 tests pass. Clippy clean. All feature combos clean.
- [x] **P5.4 (speculative)** HLA committed-belief probe (F4) — perturb committed HLA state, re-resolve one step, measure drift. Carries R344 null-result caveat. **DONE — Plan 414 (2026-07-09):** unblocked after R344 re-validation confirmed `CommittedBlendState` is a one-shot closed-form map (not an iterative attractor — R344's flip-flop pathology is impossible by construction). Implemented as `committed_blend_pi_sensitivity` behind opt-in `hla_committed_belief_probe` feature. **Design correction:** the probe perturbs π (parameters), not the output state — so it does NOT use the `RenoiseCeProbe` trait (which perturbs output). It's a bespoke π-sensitivity function. The issue's proposed gate (`realized drift < cached lipschitz_bound · δ`) was **incorrect** — the cached `lipschitz_bound` is for z-sensitivity (input → output), not π-sensitivity. Plan 414 corrects this: computes the π-specific bound on-the-fly (`L_π = max_j (1/τ)·σ_j·(1−σ_j)·‖f_j(z)‖`). GOAT G1+G1b+G2+G2b+G3+G4+G5 ALL PASS (G1: 1000/1000 random configs 0 violations; G2: NaN detection + under-reported-Lipschitz detection; G3: 13/13 existing tests no-regression; G4: 0 allocs/1000 calls; G5: p50=3.042µs<5µs release). **Stays OPT-IN** — diagnostic primitive, no runtime consumer yet. DRY refactor: extracted `apply_blended_with_pi` free function (shared by production `apply_blended` + probe). See [`.plans/414_hla_committed_belief_lipschitz_probe.md`](414_hla_committed_belief_lipschitz_probe.md).
