...
**Target:** `katgpt-rs/crates/katgpt-pruners/crates/katgpt-pruners/src/self_advantage.rs` (new module) + Cargo features `self_advantage_gate`, `product_policy_sharpen`
...
- [x] **T5.3** (speculative, blocked on T5.1 — **T5.1 COMPLETE 2026-06-17, blocker removed**) Freeze/thaw: snapshot the improvement direction vector `A(·)` per NPC personality as a versioned latent direction (BLAKE3-committed). Needs aggregation design — `A(·)` is per-step per-candidate, not a single direction vector.
  - **Implemented 2026-06-17 (overriding prior deferral verdict):** `AdvantageDirectionAccumulator` (EMA aggregator) + `AdvantageDirectionSnapshot` (BLAKE3-committed freeze/thaw artifact), feature `advantage_freeze_thaw` (opt-in). Source: `crates/katgpt-pruners/crates/katgpt-pruners/src/self_advantage.rs` (~300 LOC appended, file still < 2048 lines).
...
| T1.1–T1.4 (self_advantage) | ✅ done | `crates/katgpt-pruners/crates/katgpt-pruners/src/self_advantage.rs` — math, unit tests, property test cross-validated with SDPG `centered_log_ratio`. |
