...
### 2. crates/katgpt-core/src/flow/cache.rs — `get_or_compute` hot path
...
### 3. crates/katgpt-core/src/mux/dd_tree.rs — `collect_leaf_paths` allocations
...
### 5. crates/katgpt-core/src/mux/freeze_thaw.rs — `MuxPatternStore`
- [x] `pattern_count()` — O(1) via cached `total_patterns` field, incremented on freeze

### 6. crates/katgpt-core/src/flow/mod.rs — `from_q_values` SIMD friendliness
- [x] Chunked loop (4 cells at a time) to help LLVM auto-vectorize the max reduction

### 7. crates/katgpt-core/src/and_or/types.rs — Tree metrics
