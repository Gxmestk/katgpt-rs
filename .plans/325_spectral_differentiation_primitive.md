...
- **Complementary to DEC `exterior_derivative`** (`crates/katgpt-dec/src/operators.rs`): DEC handles arbitrary cell complexes (irregular meshes, 2D/3D grids, manifolds with boundary). This primitive handles only the 1D periodic uniform case, but does so in O(N log N) with zero topological setup — useful when the input is a flat array of equally-spaced periodic samples (e.g. time-series windows, cyclic HLA channels, ring buffers).
