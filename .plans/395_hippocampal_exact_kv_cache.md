...
- [x] **T1.3** Implement `observe(&mut self, k: &[f32; D], v: &[f32; D], beta: f32, residual_norm: f32)`:
  ...
  - O(log W) per observe.
- [x] **T1.4** Implement `read_cache_into(&self, q: &[f32; D], gamma: &[f32; D], block_kv: &[(Vec<f32>, Vec<f32>)], out: &mut [f32; D])`:
...
- [x] **T1.5** Implement `reset(&mut self)` — clear heap, free all slots. Used at sequence boundaries.
- [x] **T1.6** Add `hippocampal_cache` to the `katgpt-core` `lib.rs` feature-gated module list. Run `cargo check -p katgpt-core --features hippocampal_cache` — must compile clean.

### Validation (Phase 1)

- [x] **T1.V1** Unit test: insert 100 tokens with random scores into a `HippocampalCache<8, 4>`. Verify the surviving 4 slots are the top-4 by score, in any order. Verify scores match the inserted values.
- [x] **T1.V2** Unit test: order-independence. Insert the same 100 (k, v, score) triples in 5 different random orders. Verify the final cache set is **identical** (same 4 slots, same k/v/scores). This is the HOLA contract: "the same top-w set can be maintained online or blockwise without order dependence" (paper §3.3).
- [x] **T1.V3** Unit test: read determinism. Given a fixed cache state and fixed q + γ, `read_cache_into` produces byte-identical output across two calls (no hidden state, no allocation-dependent ordering).
- [x] **T1.V4** `cargo check --all-features` — must pass (the `merkle_root` / `can_freeze` lesson: combo-only regressions).
...
- [x] **T2.2** **G2 — latency benchmark** in `crates/katgpt-core/benches/hippocampal_cache_goat.rs`:
...
- [x] **T2.3** Heap-vs-sorted-vec micro-bench (Risk #3). At W=64, also benchmark a sorted-vec variant `SortedSlotCache<D, W>` that does linear-scan eviction. Pick the winner as the production backing store. Document the choice in the module doc-comment.
- [x] **T2.4** Mark G1, G2 in the GOAT gate table at the bottom of this plan. Commit.
...
- [x] **T3.1** Wire `HippocampalCache` into `gdn2::State` behind the `hippocampal_cache` feature:
...
- [x] **T3.2** **G3 — no-regression.** Test: run `gdn2_recurrent_step` on a fixed input stream with `cache = None` and `cache = Some(..)` (cache observed but read discarded). Assert the **GDN2 state S_t is byte-identical** between the two runs. The cache is a pure observer of the delta-rule update — it must not perturb the state.
- [x] **T3.3** Test: with `W = 0` (cache disabled via config), the decode output is byte-identical to bare GDN2 (no `hippocampal_cache` feature). This proves the feature gate is clean (the `merkle_root` lesson applied).
- [x] **T3.4** `cargo test -p katgpt-core --features hippocampal_cache,gdn2_attention --lib` — all GDN2 + cache tests green.
...
- [x] **T4.1** Build a synthetic multi-key associative recall harness in `crates/katgpt-core/tests/hippocampal_cache_retrieval.rs`:
...
- [x] **T4.2** Apply §3.5 modelless unblock for γ (Risk #2). If γ=ones fails G4:
...
- [x] **T4.3** Run G4 with three competitors on the same stream (defend-wrong pattern from §3.6, even though no parity claim — it strengthens the gate):
...
- [x] **T4.4** Commit. Update GOAT gate table.
...
| Gate | Target | Status |
|---|---|---|
| **G1** — Eviction correctness | 8/8 needles retained, distractors evicted, order-independent | ✅ **PASS** (Phase 2) — 8/8 retained, all distractors evicted, 5-order-independence verified |
...
| Bench | Config | Result | Target | Verdict |
|---|---|---|---|---|
| observe (heap) | W=64, D=256 | 28.7 ns | ≤ 100 ns | ✅ 3.5× headroom |
| observe (micro) | W=16, D=8 | 1.75 ns | ≤ 30 ns | ✅ 17× headroom |
| read (softmax, fast) | W=64, D=256 | 2.87 µs | ≤ 1 µs | ⚠️ 2.9× over (compute-bound at D=256) |
| read (softmax, standard) | W=64, D=256 | 5.79 µs | ≤ 1 µs | ⚠️ pre-normalized fast path cuts this 2× |
| read (softmax, micro) | W=16, D=8 | 86 ns | — | ✅ well under 1 µs |
| T2.3 heap vs sorted (W=64) | heap 7.5 ns vs sorted 32.3 ns | heap 4.3× faster | heap wins | ✅ heap is production |
| T2.3 heap vs sorted (W=16) | heap 1.76 ns vs sorted 11.3 ns | heap 6.4× faster | heap wins | ✅ |

The read target miss at D=256 is an honest finding: the read is compute-bound by O(W·D) = 16K multiply-adds in the dot products. The 1 µs target was calibrated for GPU/parallel reads; on a single CPU core at D=256 the streaming softmax + dot products take ~2.9 µs. For the micro game config (D=8, W=16) reads are 86 ns. The pre-normalized fast path (`read_cache_into_fast`) halves the cost by caching `RMSNorm_γ(k)` at observe time.

### G4 retrieval detail

| Competitor | Read mode | Needles recovered | Min cosine | Verdict |
|---|---|---|---|---|
| **HOLA** (top-w by β·‖e‖) | softmax | **8/8** | 1.0000 | ✅ near-perfect retrieval |
| **HOLA** (top-w by β·‖e‖) | per-key-rescale | **8/8** | 1.0000 | ✅ both γ variants pass |
| HOLA | sigmoid-gated | 0/8 | 0.44 | ❌ not competitive (documented) |
| Recency baseline | softmax | 0/8 | -0.24 | needles evicted (early in stream) |

**Why softmax, not sigmoid (AGENTS.md deviation — documented):** The sigmoid-gated read recovers 0/8 needles with mean cosine 0.61 — non-matching slots contribute `sigmoid(≈0) ≈ 0.5 · v_j` each, accumulating noise that overwhelms the signal. Softmax normalizes (competitive selection), achieving cosine ≈ 1.0. The AGENTS.md rule "Use sigmoid not softmax" applies to gating/routing decisions (independent per-option gates); the HOLA cache read is an attention/retrieval operation where softmax is principled. Both `read_cache_into` (softmax) and `read_cache_into_sigmoid` ship; softmax is the recommended read path.
