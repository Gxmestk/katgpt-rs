...
**Status:** DONE — G1–G4 PASS (mechanistic); G5 measured FAIL on synthetic data (Bench 553 in riir-ai, 2026-07-18). **POST-ISSUE 554 CORRECTION (2026-07-18):** the G5 root cause was mis-attributed to "synthetic data has no signal"; the actual root cause is riir-ai Issue 554 — `DualLeoTrainer::apply_leo_last_layer_update` is a no-op stub, so LEO never updates during training. The 0.50% baseline is argmax over frozen-noise LEO. Re-run of Bench 553 must wait on Issue 554 (the actual unblock), not Issue 552 (GPU + civ trajectories, necessary but not sufficient). See [`.benchmarks/467_qgf_dual_leo_oracle_goat.md`](../.benchmarks/467_qgf_dual_leo_oracle_goat.md) + [riir-ai `.benchmarks/553`](../riir-ai/.benchmarks/553_qgf_dual_leo_oracle_g5.md) + [riir-ai Issue 554](../riir-ai/.issues/554_dual_leo_trainer_backprop_noop.md).
**Branch:** `develop`
**Repo:** `katgpt-rs`
...
Shipped `DualLeoOracle` as QGF's 3rd oracle — LEO+UVFA Q-gradient fusion at the gradient level. Plan 460's max-pool washout lesson is encoded as a design invariant (no operator between the `DualLeoMixer::combine_into` and the QGF consumer). G1–G4 PASS mechanistically; **G5 measured FAIL on synthetic data (Bench 553)** — dual scored 0.00% vs single 0.50% on T7 Go puzzles; the correctness invariant (b ≡ a) held bit-identically, confirming the mechanism. **POST-ISSUE 554 CORRECTION:** the G5 root cause was mis-attributed to "synthetic data has no signal"; the actual root cause is riir-ai Issue 554 (`DualLeoTrainer::apply_leo_last_layer_update` is a no-op stub; LEO never trains). Re-run of Bench 553 must wait on Issue 554 (the actual unblock), not Issue 552 (necessary but not sufficient). Primitive stays opt-in with documented unproven G5.
