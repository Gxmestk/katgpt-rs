...
| Issue | Verdict |
|-------|---------|
| Target repo `crates/katgpt-core/src/dirichlet.rs` (public MIT) | ❌ **Wrong per `003`.** Functor arithmetic is a latent operation (projection direction + sigmoid gate). `003` explicitly: "Latent-operation internals → riir-ai internal." Must ship to riir-ai. |
...
**Target repo (corrected):** `riir-ai/crates/riir-engine/src/latent_functor/` (private, per `003`). NOT `crates/katgpt-core/src/dirichlet.rs` (public) — the original target was wrong.
