...
**Target:** extend `sample_from_distribution` in `crates/katgpt-core/crates/katgpt-core/src/speculative/sampling.rs` with a QMC-aware variant that carries the rescaled local coordinate `u_t`.
...
- [x] **katgpt-rs QmcHalter** (R205 consumer) — sample-efficiency-aware halter that estimates coverage from the actual QMC point set vs the union-bound ceiling `min(1, k·p)`. **Fusion E per R367 §2.3.**
      **✅ DONE (2026-07-04)** — shipped directly under Plan 367 (per user instruction "Do NOT start a new plan"). New file `crates/katgpt-core/crates/katgpt-core/src/speculative/qmc_halter.rs` (gated on `qmc_sampling`, which is default-on per Plan 367 Phase 5). The sample-efficiency-aware analog of `GainCostLoopHalter` (Plan 304):
