...
- [-] **T4.5** If all four pass: add `karc_forecaster` to `crates/katgpt-core/Cargo.toml` default features. **DEFERRED (2026-07-20 update)** — Issue 185 T1+T2 closed (large-d_h Jacobi B-step implementation lands + parity tests pass at d_h=96/r=4 with 1.6e-14 agreement). T3 (the actual d_h=18_720 GOAT gate re-run) is **blocked on a faster symmetric eigensolver** — naive Jacobi at d_h=18_720 needs ~3.3e14 FLOPs one-time + cache-hostile access pattern; even d_h=4_752 exceeded a 10-minute budget. A latent sign bug in `jacobi_eigen` (Plan 308 T2.3) was also found and fixed during this work. **The compute-unblock deliberation is tracked in [Issue 186](../.issues/186_karc_t3_compute_unblock_deliberation.md)** — four paths analyzed (LAPACK `dsyevd` / in-repo Householder+QL / parallel Jacobi / gate re-spec) with a recommended decision protocol. Promotion remains DEFERRED. Algorithm + implementation are correct; the blocker is purely compute-feasibility.
