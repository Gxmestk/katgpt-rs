...
**Target:** `crates/katgpt-core/src/funcattn/mod.rs` (new module) + Cargo feature `funcattn`
**Status:** ✅ COMPLETE — Phase 1 done (T1.1–T1.5 ✅), Phase 2 done (T2.1–T2.3 ✅ G1+G4+G5 PASS), Phase 3 done (T3.1 ✅ G3 PASS — sigmoid 33% BETTER than softmax; T3.2 ✅ G2 STRICT PASS — FUNCATTN beats SDPA 10.9× and Parallax 18.4× on sinusoidal regression), Phase 4 done (T4.1–T4.4 ✅ — T4.4 closed 2026-06-19 with G6 FAIL null result, **CORRECTED 2026-07-07 per Issue 049**: G6 now PASSES after the degenerate-training-data fix; FUNCATTN 1.000 ≥ SDPA 1.000; `funcattn` **promoted to DEFAULT-ON 2026-07-07** commit `776000b9`). Phase 5 done (T5.1–T5.3 ✅ 2026-06-19) — three opt-in composition modules shipped behind `funcattn_compose` parent feature (each independently switchable: `funcattn_spectral_pre_rotate`, `funcattn_chiar_blend`, `funcattn_freeze_thaw`); NOT promoted to default/full, awaiting composition-specific GOAT gates. **All plan tasks complete.**
**Tier:** Gain → **PROMOTED to DEFAULT-ON 2026-07-07** (G6 PASSES post-Issue-049 D4 fix; 6/6 GOAT gates green; commit `776000b9`). Composition modules (T5.1-T5.3) remain opt-in behind `funcattn_compose`.
...
- [x] **T1.2** Create `crates/katgpt-core/src/funcattn/mod.rs` with the core types.
  ```rust
...
- [x] **T4.4** **Do NOT promote to default until LLM-domain token-prediction evidence exists.** This is a separate gate (deferred per Research 257 §5 Q2). **STATUS: CLOSED 2026-06-19 as G6 FAIL; CORRECTED 2026-07-07 per Issue 049 — G6 now PASSES.** The original G6 null result (FUNCATTN 0.969 < SDPA 1.000 at 600 FD-SGD steps) was a test-config artifact: `generate_pattern_dataset` admitted degenerate `a==b` constant training sequences (5/32 at V=8) that corrupted FUNCATTN's learned basis. Fix (D4): reject `a==b` (PRNG stream preserved). **With clean data, FUNCATTN reaches acc=1.000 — the plateau was never a structural ceiling.** `funcattn` is now **eligible for default-on promotion**; promotion is a human decision per T4.4 (flip feature into default list + update `.docs/01_overview.md`). See `.benchmarks/058_funcattn_goat.md` G6 Results.
...
- `crates/katgpt-core/src/funcattn/mod.rs` — new module
