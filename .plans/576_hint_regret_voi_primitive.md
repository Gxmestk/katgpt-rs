...
> **Source:** Research 496 (SPADE distillation) · Guide 340 (riir-ai)
> **Date:** 2026-08-21
...
- [ ] `learnable_band_gate(w, w_lo, w_hi, kappa) -> f32` — `σ(κ(w−w_lo))·σ(κ(w_hi−w))`; monotone ↑↓, strict (0,1), peaks at band center. Property tests + one-line Lean extension of the shipped `sigmoid_bounded` family (`.proofs/KatgptProof`).
...
- [ ] `beta_lcb_order(scores: &[(successes, fails)]) -> Vec<usize>` — ε-quantile of Beta(1+S, 1+F) descending (mirror of shipped `SelectionMode::BetaPosterior`; extract or re-implement leaf-clean — check katgpt-core first, DRY).
...
- [ ] [-] riir-ai quest-center frontier weighting (after PoC Issue 677 verdict — Guide 340 P2).
