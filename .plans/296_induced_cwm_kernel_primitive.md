...
- [x] **T2.1** `ismcts_search_with_inference<S, B>` implemented in `crates/katgpt-core/src/induced_cwm/ismcts.rs`. **Algorithm simplification**: instead of full single-tree Cowling 2012 ISMCTS, implements per-iteration determinized MCTS with root-level info-set aggregation (sample one hidden state per iteration → rollout per root action → accumulate in shared root HashMap). Sufficient for G2 gate; documented in rustdoc. Future rev can implement full single-tree if deeper-lookahead games need it.
...
- [x] **T2.4** Leduc-style IIG mock in `crates/katgpt-core/src/induced_cwm/ismcts_tests.rs` (10 tests, all pass). G2 gate test: `ismcts_picks_nonfold_at_least_70pct_when_strong_hand` verifies non-fold ≥ 70% when posterior P(strong) ≥ 0.7. Plus 9 supporting tests (NodeStats, InformationSet, action_hash stability, determinism, canonical_bytes constancy).
