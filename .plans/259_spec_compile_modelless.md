...
- [x] **T1**: Create `crates/katgpt-pruners/src/spec_compile/types.rs` — `SpecRule` type with token pattern + `CompactBitmap` allowed/blocked sets, `CompiledSpec` wrapper, `SpecType` enum
- [x] **T2**: Create `crates/katgpt-pruners/src/spec_compile/compiler.rs` — `SpecCompiler` struct that compiles spec string into `Vec<SpecRule>`, handles classification/extraction/format specs, pattern-based label extraction
- [x] **T3**: Create `crates/katgpt-pruners/src/spec_compile/pruner.rs` — `impl ConstraintPruner for CompiledSpec` with O(1) bitmap lookup per token, batch validation, zero-alloc hot path
- [x] **T4**: Create `crates/katgpt-pruners/src/spec_compile/screening.rs` — `impl ScreeningPruner for CompiledSpec` with relevance scoring for ambiguous specs
...
- [x] **T6**: Create `crates/katgpt-pruners/src/spec_compile/marginals.rs` — `SpecMarginals` type with token probability bias from spec, integration with DDTree marginal interface
...
- [x] **T9**: Create `crates/katgpt-pruners/src/spec_compile/dfa.rs` — `SpecDFA` type extending PartialParser for arbitrary formats, DFA state transitions from format descriptions, token-to-character-set mapping
...
- [x] **T12**: Create `crates/katgpt-pruners/src/spec_compile/proof.rs` — `SpecProof` type with BLAKE3 commitment of compiled spec rules, `SpecCommitment` struct for tamper detection
...
- [x] **T15**: Create `crates/katgpt-pruners/src/spec_compile/chain.rs` — `SpecChain` with AND/OR composition of compiled specs, bitmap intersection (AND) and union (OR), chained pruner execution
...
- [x] **T18**: Create `crates/katgpt-pruners/src/spec_compile/router.rs` — threshold-based routing: simple spec → CPU (bitmap), complex spec → SIMD (batch), fuzzy spec → GPU/ANE (ternary adapter fallback)
...
- [x] **T20**: Create `crates/katgpt-pruners/src/spec_compile/mod.rs` — module index, re-exports, `#[cfg(feature = "spec_pruner")]` and `#[cfg(feature = "spec_compile")]` gates
