...
**Target:** `katgpt-rs/crates/katgpt-core/crates/katgpt-dec/src/stokes_calculus.rs` (new file) + Cargo feature `stokes_calculus` (under `dec_operators`)
...
- [x] **T1.1** Create `katgpt-rs/crates/katgpt-core/crates/katgpt-dec/src/stokes_calculus.rs`. Add `mod stokes_calculus;` to `dec/mod.rs` behind `dec_operators` feature. Add Cargo feature `stokes_calculus = ["katgpt-core/dec_operators"]` to `katgpt-rs/Cargo.toml` (opt-in, NOT default).
