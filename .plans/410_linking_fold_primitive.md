...
| G2 detector cold-path | ✅ PASS audit-cadence budget (500 ms @ n=2×200). **Issue 050 Phase 1 optimization (2026-07-07): 408→59 ms (6.9× speedup)** via BB-skip + SoA/auto-vec Gauss. Original 50 ms @ n=2×1000 still unreachable (brute-force O(β²)) but the gap shrank 7×. | 59.2 ms measured @ n=2×200 post-opt (was 408 ms pre-opt). Minutes extrapolated @ n=2×1000. Opt-in `linking_fold_detector`. Optimization: (1) bounding-sphere pre-check skips 84.5% of cycle pairs whose Gauss integral provably rounds to 0; (2) full-SoA segment layout (separate x/y/z arrays) enables auto-vec on the inner-sy reduction. Both correctness-safe (G1 still detects Hopf, unlinked, fold-unlinks). |
