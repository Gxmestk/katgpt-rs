...
- [x] **T4.3** Add an integration path for GDN2 models in the speculative step. `speculative_step_gdn_tree` in `src/speculative/step_gdn_tree.rs` (root crate, gated `gdn_tree_verify`). Uses `forward_tree_gdn2` (new, `crates/katgpt-attn/src/gdn2/tree_forward.rs`) to process all tree nodes in one pass via tree verify, then p/q rejection sampling along the best path, then commits via sequential `forward_gdn2` replay. Also ships `build_topology_from_tree_nodes` in `crates/katgpt-core/src/gdn_tree_verify/mod.rs` (DDTree path-encoded nodes to parent-index topology).
...
- [x] **T4.3c** Hybrid QwenDeltaNet (mixed Attention + DeltaNet layers). Attention layers use per-branch sequential KV-rollback (simplest correct approach for branching trees); DeltaNet layers use tree verify via the katgpt-core primitive. Per-layer dispatch in the tree forward. **riir-ai consumer** — `forward_tree_qwen_deltanet` in `katgpt-rs/crates/katgpt-attn/src/gdn2/tree_forward.rs` (feature `gdn_tree_verify = ["deltanet_inference", "katgpt-core/gdn_tree_verify"]`). Cross-repo primitive consumption proven: state transposed `[d_v×d_k]→[d_k×d_v]` per head, per-head α/β via `recompute_cumulative_log_decay`, tree-structured conv1d windows, all matching sequential `forward_qwen_deltanet` to ≤1e-3. 3 equivalence tests pass (pure-DeltaNet chain, hybrid chain, pure-DeltaNet branching). **Full hybrid speculative pipeline tracked as riir-ai Issue 429 — FULLY RESOLVED** (issue file removed per noise-reduction rule): T4.3c.1 forward path (commit `5c5b2122`, 3 tests), T4.3c.2 commit path `commit_tree_qwen_deltanet` — sequential replay matching `speculative_step_gdn_tree`'s `commit_accepted_path_sequential` (commit `ed06451f`, 2 tests, ≤1e-5), T4.3c.3 top-level `speculative_step_qwen_deltanet_tree` orchestrating DFlash draft → DDTree build → tree verify → p/q rejection → sequential commit (commit `3b25a524`, 3 tests incl. cache-state equivalence). 8/8 `deltanet::tree_forward` tests pass. Drafter is standard `TransformerWeights` (DFlash), target is `QwenDeltaNetWeights` (hybrid) — matches the katgpt-rs cross-model-type pattern.
- [x] **T4.4** Integration tests. 2 tests in `step_gdn_tree.rs` (returns valid tokens, deterministic for same seed). 3 tests in `tree_forward.rs` (single-node, chain, finite logits). 4 tests in `crates/katgpt-core/src/gdn_tree_verify/mod.rs` (topology from DDTree: chain, branching, dedup, missing-parent panic). T4.3b added 3 more direct-equivalence tests in `tree_forward.rs` (chain matches sequential, single-layer chain matches, branching matches per-branch).
...
- [x] **T5.2 (G2 — perf)** Benchmark `crates/katgpt-core/benches/bench_424_gdn_tree_verify.rs`: ✅ See [`.benchmarks/424_gdn_tree_verify_goat.md`](../.benchmarks/424_gdn_tree_verify_goat.md). **Chain tree speedup matches paper's B200 GPU numbers**: 1.93×/2.79×/4.66×/**7.09×** at T=16/32/64/128 (paper: 1.5×/2.7×/4.6×/7.1×). Shallow (random) trees show 1.18-1.40× (sequential does less total work at depth ~log T). G2 PASSES for the algorithmically favorable case (deep trees).
