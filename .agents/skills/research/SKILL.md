...
| **Pass** | Mechanism already ships OR not relevant OR training-only AND genuinely out of scope for our model-based track (explicit justification required per §3.5 Path 0.5). | **No new research/plan files.** Verdict + one-line reason + closest shipped cousin, in conversation only. If the paper has actionable improvements (config contradiction, unmitigated failure mode, unblocker), that's **Gain**, not Pass — see §1.55. **Training-efficiency papers applicable to our model-based track (LLM/LoRA training via Plan 318) are NEVER Pass** — they're Gain → Plan in `riir-train/.plans/` per §3.5 Path 0.5 (as of 2026-08-06: modelless track near limit, model-based track actively pursued). **MANDATORY §1.55.1: add a `PASS-Redirects:` cross-reference line to the 1–3 closest shipped cousin `.research/` notes** so future greps for the arxiv ID / paper title hit the existing note instead of returning zero results. |
