...
**Complementary defense — the game-context reframe (§1 step 4, the Lonely Runner lesson):** the reverse-grep above catches papers that FILL a documented gap. It does NOT catch papers that CREATE a new capability (no documented gap exists because the capability doesn't exist yet). The Lonely Runner paper (arXiv:0710.4495) had zero documented gaps to fill — "guaranteed NPC individuality moments" was a new behavior class, not a gap fix. The reverse-grep returned clean → false PASS. The game-context reframe (§1 step 4) is the complementary defense: it catches papers where the value is a NEW per-entity behavior signal / crowd pattern / selling point, not a gap fill. **Both defenses are mandatory before PASS**: reverse-grep for gap-fillers (§1.55.2), game-context reframe for new-capability papers (§1 step 4). Skipping either produces a false PASS.
