...
| **G2** perf | `from_states` over belief-realistic trajectory (100-step × dim=8) < 5 µs | **3.04 µs** at belief 100×8; `bifurcation_ratio` 42 ns at belief scale. Sweep across dim=32 (9.5 µs) and dim=768 (206 µs) scales linearly — those are offline-diagnostic workloads, not hot-path. | ✅ PASS |
...
1. **Does not prove the paper's transformer-layer curvature result transfers
   to belief emotion trajectories.** The gate uses a synthetic 2-D construction,
   not actual belief evolution. The realistic damped-oscillation sanity test
   ...
   a threshold for production belief routing.
...
Gate workload: belief-realistic trajectory (100-step × dim=8) — the actual
...
|---|---|---|---|---|
| belief-short (1 tick)    |  20 |   8 |   708 ns |  42 ns |
| **belief-medium (G2 target)** | **100** |   **8** | **3.04 µs** ✅ | **42 ns** ✅ |
| belief-long (crowd audit) | 1000 |   8 |  29.6 µs |  42 ns |
...
`bifurcation_ratio` at belief scale is 42 ns — essentially free (single pass,
no acos). `from_states` at belief scale is 3.04 µs — comfortably under the 5 µs
...
5. **Gate workload honest reframe**: the 5 µs target is met at belief-realistic
...
100×32. Not done in this plan — the belief use case (dim=8) already meets the
gate at 3.04 µs.
...
All GOAT gates PASS (G1 correctness 22/22 tests, G2 perf 3.04 µs at belief
