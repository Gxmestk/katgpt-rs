...
- That the gain holds for **real trained networks** (`CivLeoNet` + a UVFA network). The mock heads are designed-adversarial (sharp UVFA, broad multimodal LEO); real networks may show a stronger or weaker delta. Real-network evidence remains a separate follow-up (riir-games-civ wiring).
  **[Issue 549 follow-up result, 2026-07-18:]** the synthetic gain **DID NOT survive** contact with untrained xavier-init CivLeoNet + CivLeoUVFA — G5' drops from 31.5% (synthetic, α=0.10) to 3.3% (untrained real, α=0.10). The Plan 460 two-failed-gates stop rule triggered. Proposal 028 (civ flow-field wiring) is REJECTED on untrained networks; the gate-crossing requires **trained** networks, tracked in `riir-ai/.issues/552`. The mechanism itself stays sound (G1 PASS on both synthetic and real untrained) and the postmax API stays shipped — `DualLeoMixer` now has a 4th consumer via Plan 467 `DualLeoOracle` (QGF test-time fusion, orthogonal axis). **Reading Plan 460 as "the postmax mechanism works on civ" without this caveat is misleading; it works on synthetic adversarial heads only. Trained-network evidence is the open question.** See `riir-ai/.benchmarks/550_postmax_real_network_goat.md` for the full real-network FAIL report.
