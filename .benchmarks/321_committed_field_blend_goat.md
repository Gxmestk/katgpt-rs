...
**Feature:** `committed_field_blend = ["personality_composition"]` (DEFAULT-ON since 2026-06-28; Issue 005 `committed_field_blend_default_on_promotion` executed, closed + removed; this benchmark is the canonical record)
...
**PROMOTED to default-on** (2026-06-28; Issue 005 `committed_field_blend_default_on_promotion` executed, closed + removed; this benchmark is the canonical record). The Cargo.toml flip landed in both `crates/katgpt-core/Cargo.toml` (default list) and root `Cargo.toml` (default list + passthrough feature). Acceptance gates A1–A4 ALL PASS: A1 default check clean on both crates (no new warnings); A2 katgpt-core `--no-default-features` clean (the closed Issue 005 added zero new errors — a pre-existing unrelated breakage in root `crates/katgpt-speculative/src/dflash.rs` no-default build was documented there and is out of scope); A3 13/13 lib tests green + G4a (apply_blended 1000 iters) = 0 allocs + G4b (commit 100 re-commits) = 0 allocs; A4 this doc + README + overview all flipped to DEFAULT-ON. The promotion is the formal flip from "opt-in" to "available by default" — the primitive code itself is untouched.
