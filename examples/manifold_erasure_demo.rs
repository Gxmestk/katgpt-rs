...
fn main() {
    ...
    println!(
        "  Trust region bound:      {:.6} (ε·r_i)",
        config.epsilon * info.local_radius
    );
    ...
    println!(
        "  e3 before: {:.6}, after: {:.6} (Δ = {:.2e})",
        x_2d[2],
        out_2d[2],
        (out_2d[2] - x_2d[2]).abs()
    );
    println!(
        "  e4 before: {:.6}, after: {:.6} (Δ = {:.2e})",
        x_2d[3],
        out_2d[3],
        (out_2d[3] - x_2d[3]).abs()
    );
    ...
    let infos =
        manifold_erasure_loop_into(&x, &gf, &pool, n, &config, 10, &mut scratch, &mut out_loop)
            .unwrap();

    let loop_align = simd_dot_f32(&out_loop, &u, d).abs();
    println!("  Rounds executed: {}", infos.len());
    println!(
        "  Target alignment after {} rounds: {:.6}",
        infos.len(),
        loop_align
    );
    println!(
        "  Total reduction: {:.1}%",
        (1.0 - loop_align / x_align) * 100.0
    );
