//! Bomberman SDAR Tournament — SdarPlayer vs all baselines (Plan 072).
//!
//! Round-robin tournament pitting 7 player types in 4-player matches:
//! Random, Greedy, Validator, HL, GZero, Rubric, SDAR
//!
//! Run: `cargo run --example bomber_10_sdar_tournament --features sdar_gate,ropd_rubric,g_zero,bomber`
//!
...
const MATCHUPS: &[MatchupSpec] = &[
    ...
    MatchupSpec {
        label: "Rubric Challenge",
        players: ["Random", "HL", "Rubric", "Validator"],
    },
    MatchupSpec {
        label: "SDAR Challenge",
        players: ["Random", "HL", "SDAR", "Validator"],
    },
    MatchupSpec {
        label: "Championship",
        players: ["GZero", "Rubric", "SDAR", "HL"],
    },
];
...
fn print_header() {
    println!();
    println!("══════════════════════════════════════════════════════════════");
    println!("  Bomber SDAR Tournament (Plan 072)");
    let matchups = MATCHUPS.len();
    let games = GAMES_PER_MATCHUP;
    println!("  Players: 7 | Matchups: {matchups} | Games: {games} each");
    println!("══════════════════════════════════════════════════════════════");
}
...
fn print_leaderboard(
    ...
    println!("```markdown");
    println!("## Bomber SDAR Tournament Results");
    println!();
